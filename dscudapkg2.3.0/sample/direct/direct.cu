...
calc_gravity_gpu(double *m, double (*x)[3], double eps, double (*a)[3], double *p, int n)
{
    ...
    if (firstcall) {
        firstcall = 0;
        safeCall(cudaMalloc((void**)&d_m, sizeof(float) * n));
        safeCall(cudaMalloc((void**)&d_x, sizeof(float) * 3 * n));
        safeCall(cudaMalloc((void**)&d_a, sizeof(float) * 3 * n));
        safeCall(cudaMalloc((void**)&d_p, sizeof(float) * n));
    }
    for (i = 0 ; i < n; i++) {
        floatbuf[i] = (float)m[i];
    }
    safeCall(cudaMemcpy(d_m, floatbuf, sizeof(float) * n, cudaMemcpyHostToDevice));
    ...
    safeCall(cudaMemcpy(d_x, floatbuf, sizeof(float) * 3 * n, cudaMemcpyHostToDevice));

    gravity_kernel<<<grids, threads>>>(d_m, d_x, (float)eps, d_a, d_p, n);

    safeCall(cudaMemcpy(floatbuf, d_a, sizeof(float) * 3 * n, cudaMemcpyDeviceToHost));
    for (i = 0 ; i < n; i++) {
        for (k = 0; k < 3; k++) {
    ...
    safeCall(cudaMemcpy(floatbuf, d_p, sizeof(float) * n, cudaMemcpyDeviceToHost));
    for (i = 0 ; i < n; i++) {
