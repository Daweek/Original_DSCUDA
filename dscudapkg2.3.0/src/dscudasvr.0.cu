...
RCMemcpyLocalP2P(RCHdr *rpkt0, RCHdr *spkt0)
{
    ...
    WARN(3, "dhandle:0x%08llx, srcadr:0x%08llx, count:%d\n",
         rpkt->dhandle, rpkt->srcadr, rpkt->count);

    err = cudaIpcOpenMemHandle(&dstadr, rpkt->dhandle, cudaIpcMemLazyEnablePeerAccess);
    check_cuda_error(err);

    WARN(3, "dstadr:0x%08llx\n", dstadr);

    err = cudaMemcpy(dstadr, (void*)rpkt->srcadr, rpkt->count, cudaMemcpyDefault);
    check_cuda_error(err);
    spkt->err = err;

    WARN(3, "will cudaIpcCloseMemHandle\n", dstadr);

    err = cudaIpcCloseMemHandle(dstadr);
    check_cuda_error(err);
