...
cudaMemcpyP2P(void *dst, int ddevid, const void *src, int sdevid, size_t count)
{
...
#if 1 // use cudaMemcpy(... cudaMemcpyDefault) if ddev & sdev reside in the same server.
    ddev = Vdev + ddevid;
    sdev = Vdev + sdevid;
    ...
    if (dscudaRemoteCallType() == RC_REMOTECALL_TYPE_IBV) { // dev0 -> dev1 direct transfer.
        ...
        err = dscudaRecvP2P(ddevid, sdevid, ports);
        check_cuda_error(err, "dscudaRecvP2P");

        err = dscudaSendP2P(dscudaAdrOfUva((void*)src), sdevid,
                            dscudaAdrOfUva(dst), ddevid, count, ports);
