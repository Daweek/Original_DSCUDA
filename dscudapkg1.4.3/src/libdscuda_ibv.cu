...
static int unused_p2p_port(RCServer_t *svr, int ddevid, int sdevid);
...
static int
unused_p2p_port(RCServer_t *svr, int ddevid, int sdevid)
{
    uint16_t port;
...
dscudaRecvP2P(int ddevid, int sdevid, uint16_t *p2pports)
{
    ...
    for (int i = 0; i < dstdev->nredundancy; i++, dstsvr++) {
        ...
        spkt->port = p2pports[i] = unused_p2p_port(dstsvr, ddevid, sdevid);
