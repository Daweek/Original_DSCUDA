...
setupConnection(int idev, RCServer_t *sp)
{
    ...
    sprintf(buf, "%s:%d (program ID %d) ", sp->ip, cid, pgid);

#elif 1 // TCP w/o portmapper, i.e., a fixed port.

    struct sockaddr_in sockaddr;
    struct hostent *hent;
    int sock = RPC_ANYSOCK;
    int port = (in_port_t)RC_IP_PORT_BASE + cid;
    ...
    Clnt[idev][id] = clnttcp_create(&sockaddr,
                                    ...
                                    &sock,
                                    RC_BUFSIZE, RC_BUFSIZE);
    sprintf(buf, "%s:%d (port %d) ", sp->ip, cid, port);
    ...
    switch (kind) {
      case cudaMemcpyDeviceToHost:
        ...
        for (int i = 0; i < vdev->nredundancy; i++, sp++) {
            ...
            xdr_free((xdrproc_t)xdr_dscudaMemcpyD2HResult, (char *)d2hrp);
        }
        break;
