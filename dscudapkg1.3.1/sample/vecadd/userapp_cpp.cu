...
#pragma dscuda cpp begin


#define KERNELCALLER(hname, kname, g, t, a, b, c) \
    void                                 \
    hname(float *a, float *b, float *c)         \
    {                                           \
        int nth = 4;                            \
        dim3 t(nth, 1, 1);                      \
        dim3 g((N+nth-1)/nth, 1, 1);            \
        kname<<<g, t>>>(a, b, c);               \
    }                                           \

KERNELCALLER(vecAddCaller, vecAdd, grids, threads, d_a, d_b, d_c)
