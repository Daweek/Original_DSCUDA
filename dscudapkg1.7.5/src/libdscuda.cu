                        ...
                        size_t size)
{
    ...
    for (int i = 0; i < nredundancy; i++) {
        err = dscudaBindTexture(moduleid[i], texname, (void *)devPtr, size, (RCtexture)texbuf, offset,
...
cudaError_t
dscudaSortByKey(const int nitems, uint64_t *key, int *value)
{
    cudaError_t err = cudaSuccess;
    int vi = vdevidIndex();
    int vdevid = Vdevid[vi];
    ...
    initClient();
    WARN(3, "dscudaSortByKey(%d, 0x%08lx, 0x%08lx)...", nitems, key, value);

    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_PACKET_BUF(DscudaSortByKey, vdevid, i);
