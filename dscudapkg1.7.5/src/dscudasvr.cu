...
RCDeviceSetSharedMemConfig(RCHdr *rpkt0, RCHdr *spkt0)
{
    ...
    err = cudaDeviceSetSharedMemConfig(rpkt->config);
    check_cuda_error(err);
    spkt->err = err;
    WARN(3, "done.\n");
...
static int
RCDscudaSortByKey(RCHdr *rpkt0, RCHdr *spkt0)
{
    SETUP_PACKET_BUF(DscudaSortByKey);

    WARN(3, "dscudaSortByKey(\n");
    if (!dscuContext) createDscuContext();
