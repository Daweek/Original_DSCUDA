...
#if _DSCUDA_RUNTIME_API_LAUNCH // use runtime API
    cudaError_t err;
    void **hostptr = (void **)Symbol2AddressTable[symbolname];
    if (!hostptr) {
        WARN(0, "global symbol %s did not match with any of the symbols.\n", symbolname);
        exit(1);
    }
...
#if 0 // NG
    err = cudaGetSymbolAddress((void **)dptr, *hostptr);
    WARN(2, "getGlobalSymbol symbol:%s  address:0x%016llx  device address:0x%016llx\n",
         symbolname, hostptr, *dptr);
    check_cuda_error(err);
#else // should be OK.
...
RCDscudaLaunch(RCHdr *rpkt0, RCHdr *spkt0)
{
    ...
    if (!adr) {
        ...
        switch (nmatch) {
          case 0:
            WARN(0, "%s did not match with any of the symbols.\n", restr);
            exit(1);
            break;
          case 1:
            break;
          default:
            WARN(0, "%s matched with multiple symbols.\n", restr);
            exit(1);
            break;
        }
    }
...
createSymbol2AddressTable(char* objfile)
{
    ...
    sprintf(linebuf, "%s/%s", Dscudasvrpath, objfile);
    WARN(3, "symbol2address table of file:%s\n", linebuf);
    sprintf(cmd, "nm %s | awk '{if (2 < NF) {print $1, $3}}'", linebuf);
    outpipe = popen(cmd, "r");
    if (!outpipe) {
