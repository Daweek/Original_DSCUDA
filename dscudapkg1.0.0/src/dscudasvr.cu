...
dscudaResult *
dscudawriteid_1_svc(RCsize count, RCadr dst, RCbuf src, struct svc_req *sr)
{
    static dscudaResult res;

    if (!rcuContext) createRcuContext();

    // do something for received data in 'src' buf.

    res.err = 0;
    WARN(3, "dscudaWrite(%d, 0x%08llx, 0x%08llx)\n",
         count, (unsigned long)dst, src);
    return &res;
}

dscudaReadResult *
dscudareadid_1_svc(RCsize count, RCadr src, struct svc_req *sr)
{
    static RCsize maxcount = 0;
    static dscudaReadResult res;
    ...
    //    for (int i = 0; i < count; i++) {
        //        res.buf.RCbuf_val[i] = i;
    //    }
    res.err = 0;
    WARN(3, "dscudaRead(%d, 0x%08llx, 0x%08llx)\n",
         count, (unsigned long)res.buf.RCbuf_val, src);
    return &res;
}
...
ibvMemcpyH2D(IbvHdr *rpkt0, IbvHdr *spkt0)
{
    ...
    char *rbuf = (char*)&rpkt->srcbuf;
    for (int i = 0; i < rpkt->count; i++) {
        fprintf(stderr, "%02d:%03d\n", i, rbuf[i]);
    }
#endif
