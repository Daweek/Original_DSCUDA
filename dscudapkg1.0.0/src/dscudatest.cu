...
static TestMode testmode[] =
	...
	showstatus,  "shows GPU status.",
        sendperf,    "measure send (local->remote) performance.",
        receiveperf, "measure receive (local<-remote) performance.",
...
receiveperf(int argc, char **argv)
{
    ...
    for (sized = 4096.0; sized < maxsize; sized *= ratio) {
        size = (size_t)sized;

	get_cputime(&lt, &st);
	for (j = 0; j < nloop/size; j++) {
            dscudaRead(size, dst, src);
	}
	get_cputime(&lt, &st);
	printf("%d byte    %f sec    %f MB/s\n",
               size, lt, nloop/MEGA/lt);
