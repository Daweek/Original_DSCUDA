...
static int unused_p2p_port(RCServer_t *svr);
...
static int
unused_p2p_port(RCServer_t *svr)
{
    uint16_t dummyport;

    // !!! this is just for test.
    // !!! current implementation returns a constant port.
    dummyport = 2 * RC_DAEMON_IP_PORT - svr->port;
...
dscudaSendP2P(void *sadr, int sdevid, void *dadr, int ddevid, size_t count, uint16_t *p2pports)
{
    ...
    for (int i = 0; i < dstdev->nredundancy; i++, dstsvr++) {
        SETUP_IBV_PACKET_BUF(DscudaSendP2P, sdevid, 0);
        spkt->count = count;
        spkt->srcadr = (RCadr)sadr;
        spkt->dstadr = (RCadr)dadr;

        //        spkt->dstdevid = ddevid;
        strncpy(spkt->dstip, dstsvr->ip, RC_HOSTNAMELEN);
...
dscudaRecvP2P(int ddevid, int sdevid, uint16_t *p2pports)
{
    ...
    for (int i = 0; i < dstdev->nredundancy; i++, dstsvr++) {
        SETUP_IBV_PACKET_BUF(DscudaRecvP2P, ddevid, i);
        spkt->port = p2pports[i] = unused_p2p_port(dstsvr);
...
{
#if 1 // dev0 -> dev1 direct transfer.
