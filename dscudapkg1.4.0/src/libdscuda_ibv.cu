...
cudaThreadSynchronize(void)
{
    ...
    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_IBV_PACKET_BUF(ThreadSynchronize, Vdevid[vdevidIndex()], i);
        WARN(3, "spktsize:%d\n", spktsize);
        WARN(3, "Vdevid[vdevidIndex()]:%d\n", Vdevid[vdevidIndex()]);
...
cudaThreadExit(void)
{
    ...
    Vdev_t *vdev = Vdev + Vdevid[vdevidIndex()];
    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_IBV_PACKET_BUF(ThreadExit, Vdevid[vdevidIndex()], i);
        WARN(3, "spktsize:%d\n", spktsize);
        WARN(3, "Vdevid[vdevidIndex()]:%d\n", Vdevid[vdevidIndex()]);
...
cudaGetErrorString(cudaError_t error)
{
    ...
    Vdev_t *vdev = Vdev + Vdevid[vdevidIndex()];
    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_IBV_PACKET_BUF(GetErrorString, Vdevid[vdevidIndex()], i);
...
cudaRuntimeGetVersion(int *runtimeVersion)
{
    ...
    Vdev_t *vdev = Vdev + Vdevid[vdevidIndex()];
    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_IBV_PACKET_BUF(RuntimeGetVersion, Vdevid[vdevidIndex()], i);
        WARN(3, "spktsize:%d\n", spktsize);
        WARN(3, "Vdevid[vdevidIndex()]:%d\n", Vdevid[vdevidIndex()]);
...
cudaDeviceSynchronize(void)
{
    ...
    Vdev_t *vdev = Vdev + Vdevid[vdevidIndex()];
    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_IBV_PACKET_BUF(DeviceSynchronize, Vdevid[vdevidIndex()], i);
        WARN(3, "spktsize:%d\n", spktsize);
        WARN(3, "Vdevid[vdevidIndex()]:%d\n", Vdevid[vdevidIndex()]);
...
cudaMalloc(void **devAdrPtr, size_t size)
{
    ...
    Vdev_t *vdev = Vdev + Vdevid[vdevidIndex()];
    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_IBV_PACKET_BUF(Malloc, Vdevid[vdevidIndex()], i);
        spkt->size = size;
...
cudaFree(void *mem)
{
    ...
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        SETUP_IBV_PACKET_BUF(Free, Vdevid[vdevidIndex()], i);
...
cudaMemcpy(void *dst, const void *src, size_t count, enum cudaMemcpyKind kind)
{
    ...
    switch (kind) {
      case cudaMemcpyDeviceToHost:
        err = cudaMemcpyD2H(dst, src, count, vdevid);
        break;
      case cudaMemcpyHostToDevice:
        err = cudaMemcpyH2D(dst, src, count, vdevid);
        break;
      case cudaMemcpyDeviceToDevice:
        err = cudaMemcpyD2D(dst, src, count, vdevid);
        break;
