...
cudaMemcpy(void *dst, const void *src, size_t count, enum cudaMemcpyKind kind)
{
    ...
    switch (kind) {
      case cudaMemcpyDeviceToHost:
        err = cudaMemcpyD2H(dst, src, count, vdev, clnt);
        break;
      case cudaMemcpyHostToDevice:
        err = cudaMemcpyH2D(dst, src, count, vdev, clnt);
        break;
      case cudaMemcpyDeviceToDevice:
        err = cudaMemcpyD2D(dst, src, count, vdev, clnt);
        break;
      case cudaMemcpyDefault:
        WARN(0, "cudaMemcpy: 'cudaMemcpyDefault', given as the 4th argument, is not supported.\n");
        exit(1);
