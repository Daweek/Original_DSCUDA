...
check_msgsanity(struct ibv_wc *wc, bool is_server)
{
    ...
    switch (wc->opcode) {
      case IBV_WC_RECV:
        fprintf(stderr, "completion RECV message");
        exit_on_err = true;
        break;
      case IBV_WC_SEND:
        fprintf(stderr, "completion SEND message");
        exit_on_err = true;
        break;
      ...
      case IBV_WC_RDMA_READ:
        ...
        break;
      default:
        fprintf(stderr, "completion of unknown type : 0x%x  status : %s\n",
...
poll_cq(void *ctx)
{
    ...
    while (1) {
        ...
        do {
            ...
            if (wc.status != IBV_WC_SUCCESS) {
                fprintf(stderr, "ibv_poll_cq() got WC with status %d, %s.\n",
                        wc.status, ibv_wc_status_str(wc.status));
                //                exit(1);
            }
            cnt++;
            //             fprintf(stderr, "%d\n", cnt);
            //            fprintf(stderr, "poll WCs.\n", cnt);
...
rdmaWaitEvent(struct rdma_event_channel *ec, rdma_cm_event_type et, int (*handler)(struct rdma_cm_id *id))
{
    ...
    while (rdma_get_cm_event(ec, &ep) == 0) {
        ...
        if (et == event_copy.event) {
            ...
            fprintf(stderr, "got an event %s, not what expected (%s). "
                    "event.status:%d\n",
