...
static void perform_remote_call(IbvConnection *conn, RCMethod *methodp, int sendsize);
...
static void
perform_remote_call(IbvConnection *conn, RCMethod *methodp, int sendsize)
{
    *methodp = RCMethodNone;
    rdmaWaitReadyToRdma(conn);
    rdmaKickoffRdma(conn, sendsize);
...
cudaGetErrorString(cudaError_t error)
{
    ...
    for (int i = 0; i < vdev->nredundancy; i++) {
        ...
        perform_remote_call(conn, &rpkt->method, spktsize);
                            ...
                            int narg, IbvArg *arg)
{
    ...
    if (autoVerb) {
        cudaArgs args2;
        args2.ibvCudaLaunchKernelArgs.moduleid = moduleid;
        args2.ibvCudaLaunchKernelArgs.kid = kid;
        args2.ibvCudaLaunchKernelArgs.kname = kname;
        args2.ibvCudaLaunchKernelArgs.gdim = gdim;
        args2.ibvCudaLaunchKernelArgs.bdim = bdim;
