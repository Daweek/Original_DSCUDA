...
dscudaLoadModule(char *modulename)
{
    ...
    for (i = 0; i < vdev->nredundancy; i++) {
        mid = dscudaLoadModuleLocal(MyIpaddr, getpid(), modulename, modulebuf, Vdevid, i);
    ...
    for (int idev = 0; idev < Nvdev; idev++) {
        ...
        for (i = 0; i < vdev->nredundancy; i++, sp++) {
            mid = dscudaLoadModuleLocal(MyIpaddr, getpid(), modulename, modulebuf, idev, i);
            // register a new module into the list,
            // and then, return a module id assigned by the server.
            if (i == 0) {
                for (j = 0, mp = Modulelist; j < RC_NKMODULEMAX; j++, mp++) {
                           ...
                           size_t count, size_t offset, enum cudaMemcpyKind kind)
{
    ...
    nredundancy = (Vdev + Vdevid)->nredundancy;
    switch (kind) {
      case cudaMemcpyHostToDevice:
        for (int i = 0; i < nredundancy; i++) {
            err = dscudaMemcpyToSymbolH2D(moduleid[i], (char *)symbol, src, count, offset, Vdevid, i);
        }
        break;
      case cudaMemcpyDeviceToDevice:
        for (int i = 0; i < nredundancy; i++) {
            err = dscudaMemcpyToSymbolD2D(moduleid[i], (char *)symbol, src, count, offset, Vdevid, i);
        }
        break;
      default:
    ...
    switch (kind) {
      case cudaMemcpyDeviceToHost:
        for (int i = 0; i < nredundancy; i++) {
            ...
            err = dscudaMemcpyFromSymbolD2H(moduleid[i], &dstbuf, (char *)symbol, count, offset, Vdevid, i);
            if (i == 0) {
      ...
      case cudaMemcpyDeviceToDevice:
        for (int i = 0; i < nredundancy; i++) {
            err = dscudaMemcpyFromSymbolD2D(moduleid[i], dst, (char *)symbol, count, offset, Vdevid, i);
        }
        break;
      default:
        WARN(0, "Unsupported value for cudaMemcpyKind : %s\n", dscudaMemcpyKindName(kind));
                                 ...
                                 size_t count, size_t offset, enum cudaMemcpyKind kind, cudaStream_t stream)
{
    ...
    switch (kind) {
      case cudaMemcpyHostToDevice:
        for (int i = 0; i < nredundancy; i++) {
            err = dscudaMemcpyToSymbolAsyncH2D(moduleid[i], (char *)symbol, src, count, offset, (RCstream)st->s[i], Vdevid, i);
        }
        break;
      case cudaMemcpyDeviceToDevice:
        for (int i = 0; i < nredundancy; i++) {
            err = dscudaMemcpyToSymbolAsyncD2D(moduleid[i], (char *)symbol, src, count, offset, (RCstream)st->s[i], Vdevid, i);
        }
        break;
      default:
        WARN(0, "Unsupported value for cudaMemcpyKind : %s\n", dscudaMemcpyKindName(kind));
    ...
    switch (kind) {
      case cudaMemcpyDeviceToHost:
        for (int i = 0; i < nredundancy; i++) {
            ...
            err = dscudaMemcpyFromSymbolAsyncD2H(moduleid[i], &dstbuf, (char *)symbol, count, offset, (RCstream)st->s[i], Vdevid, i);
            if (i == 0) {
                memcpy(dst, dstbuf, count);
            }
            else if (bcmp(dst, dstbuf, count) != 0) {
      ...
      case cudaMemcpyDeviceToDevice:
        for (int i = 0; i < nredundancy; i++) {
            err = dscudaMemcpyFromSymbolAsyncD2D(moduleid[i], dst, (char *)symbol, count, offset, (RCstream)st->s[i], Vdevid, i);
        }
        break;
      default:
        WARN(0, "Unsupported value for cudaMemcpyKind : %s\n", dscudaMemcpyKindName(kind));
