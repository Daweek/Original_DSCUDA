...
cudaError_t
cudaMemcpyLocalP2P(void *dst, const void *src, size_t count, int vdevid)
{
    cudaError_t err = cudaSuccess;
    Vdev_t *vdev;
    ...
    for (int i = 0; i < vdev->nredundancy; i++) {
        ...
        // unpack returned data.
        err = rpkt->err;
        WARN(3, "cudaMemcpy LocalP2P err : %d\n", err);
    }
...
cudaMemcpyP2P(void *dst, int ddevid, const void *src, int sdevid, size_t count)
{
...
#if 0 // This does not work for now. 'sdevid' knows 'src' but don't know 'dst',
      // and 'ddevid' knows vise versa.

    // use cudaMemcpy(... cudaMemcpyDefault) if ddev & sdev reside in the same server.
    ddev = Vdev + ddevid;
    sdev = Vdev + sdevid;
    if (sdev->server[0].ip == ddev->server[0].ip) {
        ...
        err = cudaMemcpyLocalP2P(dscudaAdrOfUva(dst), dscudaAdrOfUva((void*)src), count, sdevid);
