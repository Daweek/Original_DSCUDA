...
static void calc_gravity_gpus(double *m, double (*x)[3], double eps, double (*a)[3], double *p, int n);
...
calc_gravity_gpus(double *m, double (*x)[3], double eps, double (*a)[3], double *p, int n, int ndev_req)
{
...
#else // A test for broadcast API. practically useless for O(N^2)
      // algorithm since data transfer time is negligible.
    ...
    cudaMemcpyToAlldev(ndev, (void **)dstbuf, srcbuf[0], sizeof(float) * nj, cudaMemcpyHostToDevice);
    for (idev = 0; idev < ndev; idev++) {
      Device_t *dev = device + idev;
      for (i = 0 ; i < nj; i++) {
        for (k = 0; k < 3; k++) {
      ...
      srcbuf[idev] = dev->buf;
      dstbuf[idev] = (float *)dev->x;
    }
    cudaMemcpyToAlldev(ndev, (void **)dstbuf, srcbuf[0], sizeof(float) * 3 * nj, cudaMemcpyHostToDevice);

#endif

    // i-parallelism kernel execution.
    for (idev = 0, ioff = 0; idev < ndev; idev++, ioff += ni) {
...
#ifdef __RCUDA__
    rcudaSetErrorHandler(errhandler, (void *)&step);
#endif
...
#else
        fprintf(stderr, "usage: %s <infile> <outfile> [# of devices]\n",  argv[0]);
#endif
        exit(1);
    }
    if (3 < argc) {
