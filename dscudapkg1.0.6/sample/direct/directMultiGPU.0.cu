...
static void
calc_gravity_gpus(double *m, double (*x)[3], double eps, double (*a)[3], double *p, int n)
{
    ...
    for (idev = 0, ioff = 0; idev < ndev; idev++, ioff += ni) { // i-parallelism
...
main(int argc, char **argv)
{
    ...
    get_cputime(&lt,&st);
#if 1
    calc_gravity_gpus(mj, xj, eps, a, p, n);
#else
