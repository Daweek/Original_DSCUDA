...
#define RC_NSERVERMAX 8     // max # of servers per host.
#define RC_NDEVICEMAX 32    // max # of GPU devices per server.
#define RC_NREDUNDANCYMAX 4 // max # of redundant servers.
...
#define RC_KMODULEIMAGELEN (1024*1024)   // max length of a kernel-image.
...
#define RC_SERVER_CACHE_LIFETIME (30) // period (in second) for a module loaded by dscudasvr is cached.
#define RC_CLIENT_CACHE_LIFETIME (60) // period (in second) for a module sent by a client is cached. should be shorter enough than RC_SERVER_CACHE_LIFETIME.
