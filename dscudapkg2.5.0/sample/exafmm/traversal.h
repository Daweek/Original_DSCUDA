...
public:
  fvec4 approx(const int numTargets,
    ...
    CUDA_SAFE_CALL(cudaBindTexture(0, texCell, (uint4*)sourceCells.d(), sourceCells.size()*sizeof(CellData)));

    sourceCenter.bind(texCellCenter);
    CUDA_SAFE_CALL(cudaBindTexture(0, texCellCenter, (float4*)sourceCenter.d(), sourceCenter.size()*sizeof(fvec4)));
    Multipole.bind(texMultipole);
    CUDA_SAFE_CALL(cudaBindTexture(0, texMultipole, (float4*)Multipole.d(), Multipole.size()*sizeof(fvec4)));

    bodyPos.bind(texBody);
    CUDA_SAFE_CALL(cudaBindTexture(0, texBody, (float4*)bodyPos.d(), bodyPos.size()*sizeof(fvec4)));
    ...
    CUDA_SAFE_CALL(cudaUnbindTexture(texCell));
    sourceCenter.unbind(texCellCenter);
    CUDA_SAFE_CALL(cudaUnbindTexture(texCellCenter));
    Multipole.unbind(texMultipole);
    CUDA_SAFE_CALL(cudaUnbindTexture(texMultipole));
    bodyPos.unbind(texBody);
    CUDA_SAFE_CALL(cudaUnbindTexture(texBody));
  ...
  void direct(const int numTarget,
    ...
    CUDA_SAFE_CALL(cudaBindTexture(0, texBody, (float4*)bodyPos2.d(), bodyPos2.size()*sizeof(fvec4)));
