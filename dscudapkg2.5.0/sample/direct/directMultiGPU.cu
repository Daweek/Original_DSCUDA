...
static void errhandler(void *arg);
...
#pragma omp parallel for
	for (idev = 0; idev < ndev; idev++) {
            ...
            safeCall(cudaMalloc((void**)&dev->m, sizeof(float) * njru));
            safeCall(cudaMalloc((void**)&dev->x, sizeof(float) * 3 * njru));
            ...
            safeCall(cudaMalloc((void**)&dev->m, 2 * sizeof(float) * njru));
            safeCall(cudaMalloc((void**)&dev->x, 2 * sizeof(float) * 3 * njru));
            safeCall(cudaMalloc((void**)&dev->a, 2 * sizeof(float) * 3 * niru));
            safeCall(cudaMalloc((void**)&dev->p, 2 * sizeof(float) * niru));
...
#pragma omp parallel for private(i, k)
    for (idev = 0; idev < ndev; idev++) {
        ...
        safeCall(cudaMemcpy(dev->m, dev->buf, sizeof(float) * nj, cudaMemcpyHostToDevice));
        ...
        safeCall(cudaMemcpy(dev->x, dev->buf, sizeof(float) * 3 * nj, cudaMemcpyHostToDevice));
    }
...
#pragma omp parallel for private(i, k, ioff)
    for (idev = 0; idev < ndev; idev++) {
        ...
        ioff = ni * idev;
        cudaSetDevice(idev);
        safeCall(cudaMemcpy(dev->buf, dev->a, sizeof(float) * 3 * ni, cudaMemcpyDeviceToHost));
        for (i = 0 ; i < ni; i++) {
            for (k = 0; k < 3; k++) {
        ...
        // When the potential energy from multiple GPUs unmatched,
        // it's just ignored and the calculation is not rerun even if
        // DSCUDA_AUTOVERB is set. 

#if __DSCUDA_CLIENT__
        av_save = dscudaGetAutoVerb();
        dscudaSetAutoVerb(0);
#endif
        safeCall(cudaMemcpy(dev->buf, dev->p, sizeof(float) * ni, cudaMemcpyDeviceToHost));
#if __DSCUDA_CLIENT__
        dscudaSetAutoVerb(av_save);
#endif
...
main(int argc, char **argv)
{
    ...
    interval = 1; // !!! for debuging purpose.
