...
static void reallocAllRegions(int vdevid, int raidid);
static void saveDevMemImage(void);
static void restoreDevMemImage(void);
static cudaError_t cudaMemcpyP2P(void *dst, int ddev, const void *src, int sdev, size_t count);

int
dscudaMyServerId(void)
{
    return -1; // I'm not a server.
}
...
requestDaemonForDevice(unsigned int ipaddr, int devid, int useibv)
{
    ...
    // read dscudasvr as a binary image.
    sprintf(msg, "%s/%s.svr", Dscudasvrpath, getMyName());
    WARN(2, "server image:%s\n", msg);

    fp = fopen(msg, "r");
    if (!fp) {
        perror("requestDaemonForDevice:fopen");
        fprintf(stderr, "file to open: %s\n", msg);
        exit(1);
    }
    do {
        size = fread(buf, 1, sizeof(buf), fp);
        if (svrbufsize < svrsize + size) {
            svrbufsize = dscudaRoundUp(svrsize + size, 4096 * 16);
            svrimg = (char *)realloc(svrimg, svrbufsize);
        }
        memcpy(svrimg + svrsize, buf, size);
        svrsize += size;
    } while (0 < size);

    // send the size of the image.
    sprintf(msg, "svrsize:%d", svrsize);
    WARN(3, "%s\n", msg);
    sendMsgBySocket(dsock, msg);

    // send the image.
    sendDataBySocket(dsock, svrimg, svrsize);
    fclose(fp);
    free(svrimg);
...
static void
remote_call_error(int devid, int raidid)
{
    int vi = vdevidIndex();
    ...
    WARN(0, "Remote call error hook. "
         "No response from a server with devid:%d raidid:%d. "
         "Trying migration to another server...\n",
         devid, raidid);

    if (!autoVerb) {
        WARN(0, "Cannot migrate to a spare server cause DSCUDA_AUTOVERB not set. Abort.\n");
        exit(1);
    }

    if (NSpareServerLeft < 1) {
        WARN(0, "No spare server left in DSCUDA_SERVER_SPARE. Abort.\n");
        exit(1);
    }
    sp = SpareServer + NSpareServer - NSpareServerLeft;
    NSpareServerLeft--;

    sp->id = raidid;
    vdev->server[raidid] = *sp;
    WARN(2, "Spare server ip:%s  devid:%d activated.\n",
         dscudaAddrToServerIpStr(sp->ip), sp->cid);

    setupConnection(vdevid, vdev->server + raidid);
    reallocAllRegions(vdevid, raidid);
    restoreDevMemImage();
    dscudaVerbRecallHist();
    WARN(2, "remote_call_error:Server migration done.\n");
}
 ...
 * on each Server[i]. User see the primary UVA which is initially 
 * set to the value of the 1st element, adr[0].
 * Other adrs, i.e., Server[1..Nredunddancy-1], are hidden to the user,
...
RCuvaRegister(int devid, void *adr[], size_t size)
{
    ...
    uva->devid = devid;
    uva->size = size;
    uva->snapshot = (char*)calloc(size, sizeof(char));
    if (!uva->snapshot) {
        perror("RCuvaRegister()");
        exit(1);
    }
...
RCuvaQuery(void *adr)
{
    ...
    while (uva) {
        if ((unsigned long)uva->kadr <= ladr &&
            ladr < (unsigned long)uva->kadr + uva->size &&
            uva->devid == devid) {
            return uva;
        }
        uva = uva->next;
...
initEnv(void)
{
    ...
    char buf[1024 * RC_NVDEVMAX], *savep;
    RCServer_t *sp;
    Vdev_t *vdev;
    ...
    if (env) {
        ...
        Nvdev = 0;
        ip = strtok_r(buf, " ", &savep); // a list of IPs which consist a single vdev.
        while (ip) {
            strcpy(ips[Nvdev], ip);
            Nvdev++;
        ...
        for (i = 0; i < Nvdev; i++) {
            ...
            vdev = Vdev + i;
            ip = strtok_r(ips[i], ",", &savep); // an IP (optionally with devid preceded by a colon) of
    ...
    WARN(2, "DSCUDA Server\n");
    vdev = Vdev;
    for (i = 0; i < Nvdev; i++) {
        WARN(2, "  virtual device%d\n", i);
        sp = vdev->server;
    ...
    // DSCUDA_SERVER_SPARE
    env = getenv("DSCUDA_SERVER_SPARE");
    if (env) {
        if (sizeof(buf) < strlen(env)) {
            WARN(0, "initEnv:evironment variable DSCUDA_SERVER_SPARE too long.\n");
            exit(1);
        }
        strncpy(buf, env, sizeof(buf));
        ip = strtok_r(buf, " ", &savep); // split into each device name.
        while (ip) {
            SpareServer[NSpareServer].ip = dscudaServerNameToAddr(ip);
            SpareServer[NSpareServer].cid = dscudaServerNameToDevid(ip);
            NSpareServer++;
            ip = strtok_r(NULL, " ", &savep);
        }
        NSpareServerLeft = NSpareServer;
    }

    WARN(2, "DSCUDA Server Spare\n");
    for (i = 0; i < NSpareServer; i++) {
        RCServer_t *sp = SpareServer + i;
        WARN(2, "    %s:%d\n", dscudaAddrToServerIpStr(sp->ip), sp->cid);
    }
    ...
    if (env) {
        ...
        strncpy(Dscudasvrpath, ".", sizeof(Dscudasvrpath));
    }

    // DSCUDA_USEGPUDIRECT2
    env = getenv("DSCUDA_USEGD2");
    if (env && atoi(env)) {
        WARN(2, "Use GPU Direct ver2 (direct P2P).\n");
        UseGD2 = 1;
    }
    else {
        WARN(2, "Do not use GPU Direct ver2 (direct P2P).\n");
        UseGD2 = 0;
    }
...
{
    WARN(3, "dscudaSetAutoVerb(%d)\n", verb);
    autoVerb = verb;
    return;
}

int
dscudaGetAutoVerb(void)
{
    WARN(3, "dscudaGetAutoVerb()\n");
    return autoVerb;
}
...
cudaMemcpyH2D(void *dst, const void *src, size_t count, int vdevid)
{
    cudaError_t err = cudaSuccess;
    Vdev_t *vdev;
    RCuva *duva = RCuvaQuery(dst);
    int duvaoff = (char *)dst - (char *)duva->kadr;

    WARN(3, "H2D dst:0x%016llx\n", dst);
    ...
    for (int i = 0; i < vdev->nredundancy; i++) {
        ...
        spkt->dstadr = duvaoff + (RCadr)duva->adr[i];

        WARN(3, "spktsize:%d  count:%d\n", spktsize, count);
        WARN(3, "H2D adr[%d]:0x%016llx  duvaoff:0x%016llx\n",
             i, spkt->dstadr, duvaoff);

#if 1
        memcpy(&spkt->srcbuf, src, count);
        perform_remote_call(conn, &rpkt->method, spktsize, spkt->method);

#else   // this does not work for large 'count'.
        // ibv_post_send() fails due to memory allocation error.
        // I don't know why.
        perform_pipelined_remote_call(conn, &rpkt->method, spktsize, spkt->method,
                                      (char *)&spkt->srcbuf, (char *)src, count);
#endif
...
cudaMemcpyD2H(void *dst, const void *src, size_t count, int vdevid)
{
    cudaError_t err = cudaSuccess;
    Vdev_t *vdev;
    RCuva *suva = RCuvaQuery((void *)src);
    int suvaoff = (char *)src - (char *)suva->kadr;

    WARN(3, "D2H src:0x%016llx\n", src);

    vdev = Vdev + vdevid;
    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_PACKET_BUF(MemcpyD2H, vdevid, i);
        spkt->count = count;
        spkt->srcadr = suvaoff + (RCadr)suva->adr[i];
        WARN(3, "spktsize:%d  count:%d\n", spktsize, count);
        WARN(3, "D2H adr[%d]:0x%016llx  suvaoff:0x%016llx\n",
             i, spkt->srcadr, suvaoff);

        if (perform_remote_call(conn, &rpkt->method, spktsize, spkt->method)) {
            // an error occured, recovered from it and then return.
            return cudaSuccess;
        }
    ...
    if (autoVerb) {
        time_t t = time(NULL);
        if (CpSavedAt == 0.0 || CpPeriod < t - CpSavedAt) {
            CpSavedAt = t;
            saveDevMemImage();
            dscudaVerbClearHist();
        }
...
cudaError_t
cudaMemcpyLocalP2P(void *dst, cudaIpcMemHandle_t shandle, size_t count, int vdevid)
{
    cudaError_t err = cudaSuccess;
    Vdev_t *vdev;

    WARN(3, "cudaMemcpyLocalP2P(0x%016llx, 0x%016llx, %d, %d)...", dst, shandle,count, vdevid);
    vdev = Vdev + vdevid;
    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_PACKET_BUF(MemcpyLocalP2P, vdevid, i);
        spkt->count = count;
        spkt->dstadr = (RCadr)dst;
        spkt->shandle = shandle;
        ...
        // unpack returned data.
        err = rpkt->err;
        WARN(3, "cudaMemcpyLocalP2P err : %d\n", err);
    }
...
unused_p2p_port(RCServer_t *dstsvr, RCServer_t *srcsvr)
{
    ...
    // swap sid & did if sid > did, so that connection from sid to did
    // and that from did to sid share the same port.
    if (sid > did) {
        int tmpid = did;
        tmpid = did;
        did = sid;
...
static cudaError_t
dscudaSendP2PInit(int ddevid, int sdevid, uint16_t *p2pports,
                  struct ibv_pd **send_pd, struct ibv_pd **recv_pd)
{
    cudaError_t err = cudaSuccess;
    ...
    for (int i = 0; i < dstdev->nredundancy; i++, dstsvr++) {
        SETUP_PACKET_BUF(DscudaSendP2PInit, sdevid, 0);
        ...
        // unpack returned data.
        err = rpkt->err;
        send_pd[i] = rpkt->send_pd;
        recv_pd[i] = rpkt->recv_pd;
        WARN(3, "dscudaSendP2PInit err:%d  send_pd:0x%x  recv_pd:0x%x\n",
             err, send_pd[i], recv_pd[i]);
    }
...
static cudaError_t
dscudaSendP2P(void *dadr, int ddevid, void *sadr, int sdevid, size_t count,
              uint16_t *p2pports, RCmr *send_mr, RCmr *recv_mr)
{
    cudaError_t err = cudaSuccess;
    Vdev_t *dstdev, *srcdev;
    RCServer_t *dstsvr, *srcsvr;
    ...
    for (int i = 0; i < dstdev->nredundancy; i++, dstsvr++) {
        ...
        if (UseGD3) {
            spkt->lkey = send_mr->lkey[i];
            spkt->rkey = recv_mr->rkey[i];
        }
...
static cudaError_t
dscudaRecvP2PInit(int ddevid, int sdevid, uint16_t *p2pports)
{
    cudaError_t err = cudaSuccess;
    Vdev_t *dstdev, *srcdev;
    RCServer_t *dstsvr, *srcsvr;
    ...
    for (int i = 0; i < dstdev->nredundancy; i++, dstsvr++) {
        SETUP_PACKET_BUF(DscudaRecvP2PInit, ddevid, i);
        ...
        // unpack returned data.
        err = rpkt->err;
        WARN(3, "dscudaRecvP2PInit err : %d\n", err);
        WARN(3, "method : %d\n", rpkt->method);
    }
...
static cudaError_t
dscudaRegisterMR(struct ibv_pd **pd, int devid, void **adr, int length, int is_send, RCmr *mr)
{
    cudaError_t err = cudaSuccess;
    Vdev_t *dev;
    RCServer_t *svr;
    ...
    for (int i = 0; i < dev->nredundancy; i++, svr++) {
        SETUP_PACKET_BUF(DscudaRegisterMR, devid, i);
        spkt->pd = pd[i];
        spkt->adr = (RCadr)adr[i];
        spkt->length = length;
        spkt->is_send = is_send;

        WARN(3, "dscudaRegisterMR spkt pd:0x%x  adr:0x%x  length:%d  is_send:%d\n",
             spkt->pd, spkt->adr, spkt->length, spkt->is_send);
...
static RCp2pconn *
RCp2pconnRegister(int ddevid, int sdevid,
                  uint16_t *ports, struct ibv_pd **send_pd, struct ibv_pd **recv_pd)
{
    int i;
    int nredundancy = dscudaNredundancy();
    RCp2pconn *conn = (RCp2pconn *)malloc(sizeof(RCp2pconn));

    if (!conn) {
        perror("RCp2pconnRegister");
    }

    conn->ddevid = ddevid;
    conn->sdevid = sdevid;
    for (i = 0; i < nredundancy; i++) {
        conn->ports[i] = ports[i];
        conn->send_pd[i] = send_pd[i];
        conn->recv_pd[i] = recv_pd[i];
    }

    conn->prev = RCp2pconnListTail;
    conn->next = NULL;
    if (!RCp2pconnListTop) { // conn will be the 1st entry.
        RCp2pconnListTop = conn;
    }
    else {
        RCp2pconnListTail->next = conn;
    }

    RCp2pconnListTail = conn;
    return conn;
}

static RCp2pconn *
RCp2pconnQuery(int ddevid, int sdevid)
{
    RCp2pconn *conn = RCp2pconnListTop;

    while (conn) {
        if (conn->ddevid == ddevid && conn->sdevid == sdevid) {
            return conn;
        }
        conn = conn->next;
    }
    return NULL; // conn not found in the list.
}


static RCmr *
RCmrRegister(struct ibv_pd **pd, int ddevid, int sdevid, void **adr, int size, int is_send)
{
    cudaError_t err;
    int i;
    int devid = (is_send ? sdevid : ddevid);
    int nredundancy = dscudaNredundancy();
    RCmr *mr = (RCmr *)malloc(sizeof(RCmr));
    ...
    err = dscudaRegisterMR(pd, devid, adr, size, is_send, mr);
    check_cuda_error(err, "dscudaRegisterMR");

    WARN(3, "RCmrRegister  lkey:0x%x  rkey:0x%x\n",
         mr->lkey[0], mr->rkey[0]);

    mr->prev = RCmrListTail;
    mr->next = NULL;
    if (!RCmrListTop) { // mr will be the 1st entry.
        RCmrListTop = mr;
    }
    else {
        RCmrListTail->next = mr;
    }
...
static RCmr *
RCmrQuery(int ddevid, int sdevid, void **adr, int is_send)
{
    RCmr *mr = RCmrListTop;
    ...
    return NULL; // mr not found in the list.
}

static cudaError_t
cudaMemcpyP2P(void *dst, int ddevid, const void *src, int sdevid, size_t count)
{
    cudaError_t err = cudaSuccess;
    int cdevid;
    Vdev_t *ddev;
    ...
    // use cudaMemcpyDeviceToDevice() if ddevid == sdevid.
    if (ddevid == sdevid) {
        cudaGetDevice(&cdevid);
        if (ddevid != cdevid) {
            cudaSetDevice(ddevid);
            //            fprintf(stderr, ">>> cdevid:%d  ddevid:%d\n", cdevid, ddevid);
        }
        err = cudaMemcpy(dst, src, count, cudaMemcpyDeviceToDevice);
        if (ddevid != cdevid) {
            cudaSetDevice(cdevid);
        }

        return err;
    }

    if (UseGD2) { // use cudaMemcpy(... cudaMemcpyDefault) if ddev & sdev reside in the same server.
        ddev = Vdev + ddevid;
        sdev = Vdev + sdevid;
        if (sdev->server[0].ip == ddev->server[0].ip) {
            WARN(4, "src ip: %s \n", dscudaAddrToServerIpStr(sdev->server[0].ip));
            WARN(4, "dst ip: %s \n", dscudaAddrToServerIpStr(ddev->server[0].ip));

            // Note that cudaMemcpy() must be issued on the destination device.
            // Otherwise it fails sometimes, for unknown reason.
            cudaGetDevice(&cdevid);
            if (sdevid != cdevid) {
                cudaSetDevice(sdevid);
            }
            err = cudaIpcGetMemHandle(&shandle, dscudaAdrOfUva((void *)src));
            check_cuda_error(err, "cudaIpcGetMemHandle");

            err = cudaMemcpyLocalP2P(dscudaAdrOfUva((void*)dst), shandle, count, ddevid);
            check_cuda_error(err, "cudaMemcpyLocalP2P");
    ...
    if (dscudaRemoteCallType() == RC_REMOTECALL_TYPE_IBV) { // dev0 -> dev1 direct transfer.

        RCmr *send_mr = NULL, *recv_mr = NULL;

        // Establish a connection between ddevid & sdevid
        RCp2pconn *conn = RCp2pconnQuery(ddevid, sdevid);
        if (!conn) {
            uint16_t ports[RC_NREDUNDANCYMAX];
            struct ibv_pd *send_pd[RC_NREDUNDANCYMAX], *recv_pd[RC_NREDUNDANCYMAX];

            err = dscudaRecvP2PInit(ddevid, sdevid, ports); // ports set.
            check_cuda_error(err, "dscudaRecvP2PInit");

            err = dscudaSendP2PInit(ddevid, sdevid, ports, send_pd, recv_pd);
            check_cuda_error(err, "dscudaSendP2PInit");

            conn = RCp2pconnRegister(ddevid, sdevid, ports, send_pd, recv_pd);
        }

        if (UseGD3) { // use RDMA from/to GPU to/from IB HCA.
            RCuva *suva, *duva;
            int is_send;

            // Register 'src' as IB memory region.
            suva = RCuvaQuery((void *)src);
            if (!suva) {
                WARN(0, "cudaMemcpyP2P: src is a host memory.\n");
                exit(1);
            }
            is_send = 1;
            send_mr = RCmrQuery(ddevid, sdevid, suva->adr, is_send);
            if (!send_mr) {
                send_mr = RCmrRegister(conn->send_pd, ddevid, sdevid, suva->adr, suva->size, is_send);
            }

            // Register 'dst' as IB memory region.
            duva = RCuvaQuery((void *)dst);
            if (!duva) {
                WARN(0, "cudaMemcpyP2P: dst is a host memory.\n");
                exit(1);
            }
            is_send = 0;
            recv_mr = RCmrQuery(ddevid, sdevid, duva->adr, is_send);
            if (!recv_mr) {
                recv_mr = RCmrRegister(conn->recv_pd, ddevid, sdevid, duva->adr, duva->size, is_send);
            }
        }

        err = dscudaRecvP2P(ddevid, sdevid, conn->ports);
        check_cuda_error(err, "dscudaRecvP2P");

        err = dscudaSendP2P(dscudaAdrOfUva(dst), ddevid,
                            dscudaAdrOfUva((void*)src), sdevid,
                            count, conn->ports,
                            send_mr, recv_mr);
        check_cuda_error(err, "dscudaSendP2P");

        return err;
    }

    // dev0 -> client -> dev1 indirect transfer.
    // note: remote call of type TCP does not support direct transfer so far.
    {
        int cdevid;
        int pgsz = 4096;
        ...
        check_cuda_error(err, "cudaMemcpy");
...
dscudaLaunchWrapper(void **kadrp, char *key)
{
    ...
    if (autoVerb) {
        cudaDscudaLaunchWrapperArgs args;
        args.kadrp = kadrp;
        args.key = key;
        dscudaVerbAddHist(RCMethodDscudaLaunch, (void *)&args);
    }
...
cudaConfigureCall(dim3 gridDim, dim3 blockDim, size_t sharedMem, cudaStream_t stream)
{
    ...
    if (autoVerb) {
        cudaConfigureCallArgs args;
        args.gdim = gdim;
        args.bdim = bdim;
        args.smemsize = sharedMem;
        args.stream = (RCstream)stream;
        dscudaVerbAddHist(RCMethodConfigureCall, (void *)&args);
    }

    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_PACKET_BUF(ConfigureCall, vdevid, i);

        memcpy(spkt->gdim, gdim, sizeof(int) * 3);
        memcpy(spkt->bdim, bdim, sizeof(int) * 3);
...
cudaSetupArgument(const void *arg, size_t size, size_t offset)
{
    ...
    if (autoVerb) {
        cudaSetupArgumentArgs args;
        args.size = size;
        args.offset = offset;
        args.arg = (void *)arg;
        dscudaVerbAddHist(RCMethodSetupArgument, (void *)&args);
    }
...
cudaError_t
cudaSetupArgumentOfTypeP(const void *arg, size_t size, size_t offset)
{
    cudaError_t err = cudaSuccess;
    int vi = vdevidIndex();
    int vdevid = Vdevid[vi];
    Vdev_t *vdev = Vdev + Vdevid[vi];
    RCbuf argbuf;
    RCuva *uva = RCuvaQuery((void *)arg);
    int uvaoff = (char *)arg - (char *)uva->kadr;

    initClient();
    WARN(3, "cudaSetupArgument(0x%llx, %d, %d)...", arg, size, offset);

    if (autoVerb) {
        cudaSetupArgumentOfTypePArgs args;
        args.size = size;
        args.offset = offset;
        args.arg = (void *)arg;
        dscudaVerbAddHist(RCMethodSetupArgumentOfTypeP, (void *)&args);
    }
    ...
    for (int i = 0; i < vdev->nredundancy; i++) {
        ...
        spkt->size = size;
        spkt->offset = offset;
        cadr = uva->adr[i] + uvaoff;;
        memcpy(&spkt->argbuf, &cadr, size);
        WARN(3, ">>> adr[%d]:0x%016llx    uvaoff:%x\n",
             i,  spkt->argbuf, uvaoff);
        spktsize += size;
        WARN(3, "spktsize:%d\n", spktsize);
...
static void RCipcHandleUnregister(RCipcHandle *ipchandle);
static RCipcHandle *RCipcHandleQuery(cudaIpcMemHandle_t handle);

static RCipcHandle *RCipcHandleListTop = NULL;
static RCipcHandle *RCipcHandleListTail = NULL;

static void
RCipcHandleUnregister(RCipcHandle *ih)
{
    cudaError_t err;

    if (ih->prev) { // reconnect the linked list.
        ih->prev->next = ih->next;
    }
    else { // ih was the 1st entry.
        RCipcHandleListTop = ih->next;
        if (ih->next) {
            ih->next->prev = NULL;
        }
    }
    ...
    else { // ih was the last entry.
        RCipcHandleListTail = ih->prev;
    }

    WARN(3, "RCipcHandleUnregister(0x%016llx)\n", ih);
    free(ih);
}

static RCipcHandle *
RCipcHandleQuery(void *adr)
{
    cudaIpcMemHandle_t h;

    RCipcHandle *ih = RCipcHandleListTop;
    RCipcHandle *ipchandle = NULL;
    ...
    while (ih) {
        ...
        if (tnow - ih->usedat > RC_CLIENT_CACHE_LIFETIME) { // clean up an unused one.
            RCipcHandleUnregister(ih);
        }
        ih = ih->next;
    }
    return ipchandle;
...
cudaIpcGetMemHandle(cudaIpcMemHandle_t *handle, void *devPtr)
{
    cudaError_t err = cudaSuccess;
    int vid = vdevidIndex();
    cudaIpcMemHandle_t handles[RC_NREDUNDANCYMAX];
    RCipcHandle *ih;
    
#if RC_CACHE_IPCMEM
    ih = RCipcHandleQuery(devPtr);
    if (ih) {
        *handle = ih->handle;
        return cudaSuccess;
    }
    ...
    initClient();
    WARN(3, "cudaIpcGetMemHandle(0x%08llx, 0x%08llx)...", (unsigned long)handle, (unsigned long)devPtr);
    Vdev_t *vdev = Vdev + Vdevid[vid];
    for (int i = 0; i < vdev->nredundancy; i++) {
        SETUP_PACKET_BUF(IpcGetMemHandle, Vdevid[vid], i);
        spkt->adr = (RCadr)devPtr;
        WARN(3, "spktsize:%d  devPtr:%d\n", spktsize, devPtr);
        ...
        handles[i] = rpkt->handle;
    }
    *handle = handles[0];

    WARN(3, "done. *handle:0x%08llx\n", *handle);

#if RC_CACHE_IPCMEM
    // register 'ih'.
    ih = (RCipcHandle *)malloc(sizeof(RCipcHandle));
    ih->handle = handles[0];
    ih->adr = devPtr;
    ih->usedat = time(NULL);
    ih->prev = RCipcHandleListTail;
    ih->next = NULL;
    if (!RCipcHandleListTop) { // ih will be the 1st entry.
        RCipcHandleListTop = ih;
    }
    else {
        RCipcHandleListTail->next = ih;
    }
    RCipcHandleListTail = ih;
    WARN(3, "RCipcHandleRegister: adr:0x%08llx\n", ih->adr);
#endif
...
static void
reallocAllRegions(int vdevid, int raidid)
{
    cudaError_t err = cudaSuccess;
    ...
    WARN(2, "reallocAllRegions vdevid:%d  raidid:%d\n", vdevid, raidid);
    while (uva) {
        if (uva->devid != vdevid) {
            uva = uva->next;
            continue;
        }
        SETUP_PACKET_BUF(Malloc, vdevid, raidid);
        spkt->size = uva->size;
        ...
        uva->adr[raidid] = (void*)rpkt->devAdr;
        WARN(2, "reallocAllRegions uva:0x%016llx  phyadr:0x%016llx\n",
             uva->kadr, uva->adr[raidid]);
        uva = uva->next;
    }
}

static void
saveDevMemImage(void)
{
    cudaError_t err = cudaSuccess;
    RCuva *uva = RCuvaListTop;
    int av = autoVerb;

    WARN(2, "saveDevMemImage()\n");
    autoVerb = 0;
    while (uva) {
        cudaMemcpy(uva->snapshot, uva->kadr, uva->size, cudaMemcpyDeviceToHost);
        uva = uva->next;
    }
    autoVerb = av;
}
...
static void
restoreDevMemImage(void)
{
    cudaError_t err = cudaSuccess;
    RCuva *uva = RCuvaListTop;
    int av = autoVerb;

    WARN(2, "restoreDevMemImage()\n");
    autoVerb = 0;
    while (uva) {
        cudaMemcpy(uva->kadr, uva->snapshot, uva->size, cudaMemcpyHostToDevice);
        uva = uva->next;
    }
    autoVerb = av;
}
...
cudaMemcpy(void *dst, const void *src, size_t count, enum cudaMemcpyKind kind)
{
    ...
    switch (kind) {
      case cudaMemcpyDeviceToHost:
        err = cudaMemcpyD2H(ldst, src, count, vdevid);
        break;
      case cudaMemcpyHostToDevice:
        err = cudaMemcpyH2D(dst, lsrc, count, vdevid);
        break;
      case cudaMemcpyDeviceToDevice:
	...
	WARN(0, "cudaMemcpy:In 32-bit environment, cudaMemcpyDefault cannot be given as arg4."
        ...
        duva = RCuvaQuery(dst);
        if (!suva && !duva) {
            WARN(0, "cudaMemcpy:both src & dst are host memory.\n");
            exit(1);
        }
...
/*
 * dbufs    : a list of destination buffers.
 * rbufs    : a list of sourse buffers.
 * counts   : a list of the byte sizes to be transferred.
 * ndev     : the number of cudaMemcpy() transfers issued.
 */
void
...
#pragma omp parallel for
    for (i = 0; i < ncopies; i++) {
        cudaMemcpy(dbufs[i], sbufs[i], counts[i], cudaMemcpyDefault);
    }
}
...
dscudaBroadcast(void **dbufs, void *sbuf, int count, int ncopies)
{
    ...
    for (lv = 1; ; lv++) {
        int ndone         = (1 << lv) - 2;        // =  0,  2,  6, 14, ...
        int ndone_prev    = (1 << (lv - 1)) - 2;  // =  -1,  0,  2, 6, ...
        int ncurrent      = (1 << lv);            // =  2,  4,  8, 16, ...
        int ncurrent_prev = (1 << (lv - 1));      // =  1, 2,  4,  8, 16, ...
        imax = ncurrent < ncopies - ndone ? ncurrent : ncopies - ndone;
#pragma omp parallel for
        for (i = 0; i < imax; i++) {
            int sid = ndone_prev + i % ncurrent_prev;
            int did = ndone + i;
            if (sid == -1) {
                WARN(3, "% 3d <- s  ", did);
            }
            else {
                WARN(3, "% 3d <- % 3d  ", did, sid);
            }
            cudaMemcpy(dbufs[did], sid == -1 ? sbuf : dbufs[sid], count, cudaMemcpyDefault);
        }
        WARN(3, "\n");
        if (ncopies <= ndone + i) {
            break;
