...
/*
 * return 0 on success, 1 if migration occured.
 */
...
static void
perform_pipelined_remote_call(TcpConnection *conn, RCMethod *methodp, int sendsize, RCMethod mthd,
                              char *sendbuf, char *srcbuf, int count)
{
    // not pipelined yet.
    perform_remote_call(conn, methodp, sendsize, mthd);
}
