...
RCMemcpyD2H(RCHdr *rpkt0, RCHdr *spkt0)
{
      ...
      // !!! in order to emulate a malfunctional GPU.
    {
        ...
        if (drand48() < 1.0/100.0 && !err_in_prev_call) {
#if 0
            char badval = lrand48() % 256;
            int badpos = lrand48() % rpkt->count;
            WARN(2, "################ bad data generatad.\n\n");

            cudaMemcpy((void *)rpkt->srcadr + badpos, &badval, 1, cudaMemcpyHostToDevice);
            cudaMemcpy(&spkt->dstbuf, (void *)rpkt->srcadr, rpkt->count, cudaMemcpyDeviceToHost);

            err_in_prev_call = 1;
#else
            WARN(2, "################ bad data generatad.\n\n");
            ((char *)&spkt->dstbuf)[lrand48() % rpkt->count] = lrand48() % 256;
            err_in_prev_call = 1;
#endif
        }
        else {
