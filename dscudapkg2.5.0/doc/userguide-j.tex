...
\vspace*{5mm}
{\LARGE for DS-CUDA version 2.5.0}\\

\vspace*{10mm}
{\large 最終更新 : 2015 年 8 月 5 日
}
\end{center}
...
のミドルウェアです。本バージョンは CUDA バージョン 7.0 で動作確認ずみですが、
CUDA バージョン 7.0 の提供するすべての機能をサポートしているわけではありません
...
\item CUDA 開発ツール (CUDA 7.0 で動作確認済)\\
...
\begin{tabular}{ll}
  ...
  DSCUDA\_USEGD2       : & 値 1 を設定すると GPU Direct ver2 を使用します (節 \ref{sec:gpudirect} 参照)。\\[3mm]
  DSCUDA\_USEGD3       : & 値 1 を設定すると GPU Direct ver3 を使用します (節 \ref{sec:gpudirect} 参照)。\\[3mm]
  \hline
  server node のみ       & \\
  \hline
...
\begin{tabular}{ll}
  ...
  DSCUDA\_CP\_PERIOD   : & 自動再計算のためのチェックポイントを設定する時間間隔を\\
                         & 指定します。単位は秒、デフォルト値は 60 です。\\[3mm]
  DSCUDA\_SERVER\_SPARE: & ライブマイグレーション用の予備サーバの IP アドレス、\\
                         & あるいはホスト名 (節 \ref{sec:migration} 参照)。\\[3mm]
...
\Subsubsection{自動再計算}\label{sec:autoverb}
...
\item[(2)] また {\tt cudaMalloc()} によって確保したデバイスメモリのイメー
    ジを、定期的にメインメモリ上に保存します (以降ではこの機能をチェッ
    クポインティングと呼びます)。チェックポインティング時にはこれまで
    の CUDA API の呼び出し履歴が不要になるため、履歴を消去します。

    チェックポインティングの時間間隔は環境変数 {\tt
      DSCUDA\_CP\_PERIOD} で指定する秒数で調整できます。すなわちデバイ
    スからホスト方向への {\tt cudaMemcopy()} によるデータ転
    送 (以降 D2H と表記します) を行う際に、前回の D2H からの経過時間が
    指定秒数を超えていた場合、チェックポインティングが発生します。

\item[(3)] GPU の計算結果を D2H によってホストへ回収する際に回収結果が
    誤っていれば、チェックポインティング時に保存したメモリイメージをデ
    バイスメモリ上に復旧し、保存しておいた CUDA API の呼び出し履歴を順
    に再実行します。
...
す。つまり GPU 上の計算結果が、直近のチェックポインティング時のメモリイ
メージと、その後の CUDA API 呼び出しだけに依存していることが必要です。

例えば GPU 上の特定の変数を毎ステップインクリメントさせるプログラムでは、
自動再計算は正しく動作しません。再計算を行った時に、その変数が余分にイ
ンクリメントされてしまいます (チェックポインティング時に変数の値は保存
されないため)。

\Subsection{サーバのライブマイグレーション}\label{sec:migration}

サーバとの通信が途絶えた場合、クライアントはそのサーバで行っていた計算
を、予備サーバ上で続行しようとします。

予備サーバはあらかじめ、その IP アドレスもしくはホスト名を、環境変
数 {\tt DSCUDA\_SERVER\_SPARE} で指定しておきます。複数のサーバを指定す
るには空白で区切って列挙します。

サーバとの通信が途絶えると、クライアントは一台の予備サーバとの接続を試
みます。接続に成功すると、直近のチェックポインティング時に保存しておい
たメモリイメージをそのサーバのデバイスメモリ上に展開し、保存しておい
た CUDA API の呼び出し履歴を順に実行します。これらが成功すると、その後
はこのサーバを用いて計算を続行します。このようなサーバの引き継ぎをライ
ブマイグレーションと呼ぶことにします。
...
\Subsection{GPU Direct への対応}\label{sec:gpudirect}

\Subsubsection{GPU Direct とは}

GPU Direct は GPU と他の PCI Express デバイスとのデータ転送の性能向上を
目的として NVIDIA が提供している規格です。現時点では version 1, 2, 3 が規
定されています。それぞれの機能は以下の通りです。

\paragraph{GPU Direct version 1:} GPU とその他の PCI Express デバイ
ス (ネットワークカード等) の間のデータ転送において、従来は両者がホスト
計算機のメインメモリ上に別個のバッファを確保していました。そのためデー
タ転送時にはバッファ間のデータコピーが必要でした。GPU Direct verson 1
は両者が使用するバッファを共通化し、バッファ間のデータコピーを不要とし
ます。この機能はメインメモリ上のバッファを ({\tt malloc()} 等ではなく)
{\tt cudaMallocHost()} で pinned メモリとして確保した場合に使えます。
CUDA 3.1 以降でサポートされています。

\paragraph{GPU Direct version 2:} GPU 間の P2P データ転送は、従来はホ
スト計算機のメインメモリを介して行われていました。GPU Direct verson 2
は二つの GPU がメインメモリを介さず直接にデータ転送を行うことを可能とし
ます。この機能は二つの GPU が共通の IO ハブに接続されている場合にのみ使
えます。CUDA 4.0 以降でサポートされています。

\paragraph{GPU Direct version 3:} GPU とその他の PCI Express デバイ
ス (ネットワークカード等) の間のデータ転送は、従来はホスト計算機のメイ
ンメモリを介して行われていました。GPU Direct version 3 は二つのデバイス
がメインメモリを介さず直接にデータ転送を行うことを可能とします。この機
能を使うと、例えばあるホスト計算機に接続された GPU から別のホスト計算機
に接続された GPU へ、それぞれのホスト計算機のネットワークカードがデータ
を直接転送できます。この機能は GPU と相手の PCI Express デバイスが共通
の IO ハブに接続されている場合にのみ使えます。また GPU と相手の PCI
Express デバイス双方のデバイスドライバに専用の機能拡張が必要です。
CUDA 5.0 以降でサポートされています。

\Subsubsection{DS-CUDA の対応状況}

DS-CUDA は GPU Direct に部分的に対応しています。対応状況は以下の通りで
す。なお通信プロトコルは InfiniBand Verb を使用することが前提で
す (TCP/IP は対応していません)。

\paragraph{GPU Direct version 1:} 対応していません。

\paragraph{GPU Direct version 2:} 対応しています。環境変数 {\tt DSCUDA\_USEGD2} の値を 1 に設定すると使
用できます (デフォルトでは使用しません)。転送元と転送先の GPU が同一の
サーバノードの共通の IO ハブに接続されている場合にのみ機能します。

\paragraph{GPU Direct version 3:} GPU 間の P2P 転送についてのみ対応して
います。ホスト計算機と GPU の間の転送については対応していません。環境変
数 {\tt DSCUDA\_USEGD3} の値を 1 に設定すると使用できます (デフォルトで
は使用しません)。動作させるためには InfiniBand HCA とそのデバイスドライ
バがGPU Direct version 3 に対応していることが必要です。また GPU のデバ
イスドライバに拡張モジュール ({\tt nv\_peer\_mem}) が必要です。
...
\item InfiniBand 環境: {\tt MLNX\_OFED\_LINUX-2.4-1.0.0-rhel6.2-x86\_64}
\item GPU ドライバ拡張モジュール: {\tt nv\_peer\_mem}

\end{itemize}

ただしデータ転送性能は GPU Direct を用いない場合よりも遅くなってしまい
ます。原因はいまのところ不明です。
...
\hline\\[-1mm]
2.5.0   & 05-Aug-2015 & サーバのライブマイグレーション機能を実装。\\
        &             & 型番の異なるGPU同士での冗長デバイス構成に対応。\\
        &             & チェックポイントにおいてデバイスメモリイメージ\\
        &             & を保存する機能を実装。\\[3mm]
2.4.0   & 27-Mar-2015 & P2P 通信を GPU Direct ver2, ver3 に対応。\\[3mm]
