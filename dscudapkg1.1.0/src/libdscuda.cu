...
vdevidIndex(void)
{
    ...
    for (i = 0; i < VdevidIndexMax; i++) {
        if (VdevidIndex2ptid[i] == ptid) {
            //   fprintf(stderr, "----------------- i:%d  VdevidIndexMax:%d   ptid:%ld  Vdevid[i]:%d\n",
            //   i, VdevidIndexMax, ptid, Vdevid[i]);
            return i;
        }
    }
...
dscudaLoadModule(char *modulename)
{
    ...
    static int firstcall = 1;
    static int modulebufsize[RC_NPTHREADMAX];
    static char *modulebuf[RC_NPTHREADMAX];

    pthread_mutex_lock(&LoadModuleMutex);
    if (firstcall) {
        firstcall = 0;
        for (i = 0; i < RC_NPTHREADMAX; i++) {
            modulebufsize[i] = 1024;
            modulebuf[i] = (char *)malloc(modulebufsize[i]);
        }
    }
    pthread_mutex_unlock(&LoadModuleMutex);
    ...
    while (!feof(fp)) {
        ...
        len = strlen(buf);
        if (modulebufoff + len > modulebufsize[vi]) {
            modulebufsize[vi] += 1024;
            modulebuf[vi] = (char *)realloc(modulebuf[vi], modulebufsize[vi]);
        }
        memcpy(modulebuf[vi] + modulebufoff, buf, len);
        modulebufoff += len;
    }
    modulebuf[vi][modulebufoff] = 0;
    fclose(fp);

#if 1
    // module loaded into modulebuf[vi][].
    // now we're going to send it to the server.
    Vdev_t *vdev = Vdev + Vdevid[vdevidIndex()];
    for (i = 0; i < vdev->nredundancy; i++) {
        mid = dscudaLoadModuleLocal(MyIpaddr, getpid(), modulename, modulebuf[vi], Vdevid[vdevidIndex()], i);
    ...
    for (int idev = 0; idev < Nvdev; idev++) {
        ...
        for (i = 0; i < vdev->nredundancy; i++, sp++) {
            mid = dscudaLoadModuleLocal(MyIpaddr, getpid(), modulename, modulebuf[vi], idev, i);
            // register a new module into the list,
            // and then, return a module id assigned by the server.
            if (i == 0) {
                for (j = 0, mp = Modulelist; j < RC_NKMODULEMAX; j++, mp++) {
    ...
    if (autoVerb) {
        cudaArgs args;
        args.cudaLoadModuleArgs.srcname = modulename;
        verbAddHist(dscudaLoadModuleId, args);
    }
                           ...
                           size_t count, size_t offset, enum cudaMemcpyKind kind)
{
    ...
    if (autoVerb && (kind == cudaMemcpyHostToDevice || kind == cudaMemcpyDeviceToDevice)) {
        cudaArgs args;
        args.cudaMemcpyToSymbolArgs.moduleid = moduleid;
        args.cudaMemcpyToSymbolArgs.symbol = symbol;
        args.cudaMemcpyToSymbolArgs.src = src;
        args.cudaMemcpyToSymbolArgs.count = count;
        args.cudaMemcpyToSymbolArgs.offset = offset;
        args.cudaMemcpyToSymbolArgs.kind = kind;
        verbAddHist(dscudaMemcpyToSymbolH2DId, args);
    }
                                 ...
                                 size_t count, size_t offset, enum cudaMemcpyKind kind, cudaStream_t stream)
{
    ...
    switch (kind) {
      case cudaMemcpyHostToDevice:
        for (int i = 0; i < nredundancy; i++) {
            err = dscudaMemcpyToSymbolAsyncH2D(moduleid[i], (char *)symbol, src, count, offset, (RCstream)st->s[i], Vdevid[vdevidIndex()], i);
        }
        break;
      case cudaMemcpyDeviceToDevice:
        for (int i = 0; i < nredundancy; i++) {
            err = dscudaMemcpyToSymbolAsyncD2D(moduleid[i], (char *)symbol, src, count, offset, (RCstream)st->s[i], Vdevid[vdevidIndex()], i);
        }
        break;
      default:
        WARN(0, "Unsupported value for cudaMemcpyKind : %s\n", dscudaMemcpyKindName(kind));
    ...
    switch (kind) {
      case cudaMemcpyDeviceToHost:
        for (int i = 0; i < nredundancy; i++) {
            ...
            err = dscudaMemcpyFromSymbolAsyncD2H(moduleid[i], &dstbuf, (char *)symbol, count, offset, (RCstream)st->s[i], Vdevid[vdevidIndex()], i);
            if (i == 0) {
                memcpy(dst, dstbuf, count);
            }
            else if (bcmp(dst, dstbuf, count) != 0) {
      ...
      case cudaMemcpyDeviceToDevice:
        for (int i = 0; i < nredundancy; i++) {
            err = dscudaMemcpyFromSymbolAsyncD2D(moduleid[i], dst, (char *)symbol, count, offset, (RCstream)st->s[i], Vdevid[vdevidIndex()], i);
        }
        break;
      default:
        WARN(0, "Unsupported value for cudaMemcpyKind : %s\n", dscudaMemcpyKindName(kind));
...
cudaSetDevice(int device)
{
    ...
    if (autoVerb) {
        cudaArgs args;
        args.cudaSetDeviceArgs.device = device;
        verbAddHist(dscudaSetDeviceId, args);
    }
