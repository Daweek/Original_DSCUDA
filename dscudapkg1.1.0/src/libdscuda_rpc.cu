...
cudaThreadExit(void)
{
    ...
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudathreadexitid_1(Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
...
cudaThreadSynchronize(void)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudathreadsynchronizeid_1(Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaThreadSetLimit(enum cudaLimit limit, size_t value)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudathreadsetlimitid_1(limit, value, Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaThreadGetLimit(size_t *pValue, enum cudaLimit limit)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudathreadgetlimitid_1(limit, Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaThreadSetCacheConfig(enum cudaFuncCache cacheConfig)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudathreadsetcacheconfigid_1(cacheConfig, Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaThreadGetCacheConfig(enum cudaFuncCache *pCacheConfig)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudathreadgetcacheconfigid_1(Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaGetLastError(void)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudagetlasterrorid_1(Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaPeekAtLastError(void)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudapeekatlasterrorid_1(Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaGetErrorString(cudaError_t error)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudageterrorstringid_1(error, Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
    }
    WARN(5, "done.\n");
...
cudaSetDeviceFlags(unsigned int flags)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudasetdeviceflagsid_1(flags, Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaDriverGetVersion (int *driverVersion)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudadrivergetversionid_1(Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaRuntimeGetVersion(int *runtimeVersion)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudaruntimegetversionid_1(Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaDeviceSynchronize(void)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudadevicesynchronize_1(Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaDeviceReset(void)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudadevicereset_1(Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaMalloc(void **devAdrPtr, size_t size)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudamallocid_1(size, Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaFree(void *mem)
{
    ...
    RCServer_t *sp = vdev->server;
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudafreeid_1((RCadr)mem, Clnt[Vdevid[vdevidIndex()]][sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaMemcpy(void *dst, const void *src, size_t count, enum cudaMemcpyKind kind)
{
    ...
    switch (kind) {
      case cudaMemcpyDeviceToHost:
        ...
        sp = vdev->server;
        for (int i = 0; i < vdev->nredundancy; i++, sp++) {
            d2hrp = dscudamemcpyd2hid_1((RCadr)src, count, Clnt[Vdevid[vdevidIndex()]][sp->id]);
            ...
            if (i == 0) {
                ...
                if (autoVerb) {
                    ...
                    verbAddHist(dscudaMemcpyD2HId, args);
                    verbRecallHist();
                    break;
                }
      ...
      case cudaMemcpyHostToDevice:
        ...
        sp = vdev->server;
        for (int i = 0; i < vdev->nredundancy; i++, sp++) {
            h2drp = dscudamemcpyh2did_1((RCadr)dst, srcbuf, count, Clnt[Vdevid[vdevidIndex()]][sp->id]);
      ...
      case cudaMemcpyDeviceToDevice:
        ...
        sp = vdev->server;
        for (int i = 0; i < vdev->nredundancy; i++, sp++) {
            d2drp = dscudamemcpyd2did_1((RCadr)dst, (RCadr)src, count, Clnt[Vdevid[vdevidIndex()]][sp->id]);
                         ...
                         RCargs args)
{
    ...
    if (autoVerb) {
        cudaArgs args2;
        args2.rpcCudaLaunchKernelArgs.moduleid = moduleid;
        args2.rpcCudaLaunchKernelArgs.kid = kid;
        args2.rpcCudaLaunchKernelArgs.kname = kname;
        args2.rpcCudaLaunchKernelArgs.gdim = gdim;
        args2.rpcCudaLaunchKernelArgs.bdim = bdim;
        args2.rpcCudaLaunchKernelArgs.smemsize = smemsize;
        args2.rpcCudaLaunchKernelArgs.stream = stream;
        args2.rpcCudaLaunchKernelArgs.args = args;
