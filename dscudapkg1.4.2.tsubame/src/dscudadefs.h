...
#define RC_NREDUNDANCYMAX 1 // max redundancy per server.
#define RC_NVDEVMAX 1048      // max # of virtual devices per client.
#define RC_NPTHREADMAX 1048  // max # of pthreads which use virtual devices.
/* 
#define RC_BUFSIZE (1024*1024) // size (in byte) of send/receive buffers for rpc.
*/
#define RC_BUFSIZE (10*1024*1024) // size (in byte) of send/receive buffers for rpc.
...
#define RC_CLIENT_CACHE_LIFETIME (720) // period (in second) for a module sent by a client is cached. should be shorter enough than RC_SERVER_CACHE_LIFETIME.
