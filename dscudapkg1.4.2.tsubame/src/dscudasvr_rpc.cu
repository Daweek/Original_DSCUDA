...
dscudamallocid_1_svc(RCsize size, struct svc_req *sr)
{
    ...
    fprintf(stderr, "0x%08llx, %d devadr:0x%08llx\n", &devadr, size, devadr); fflush(stderr);
    //---> moikawa
#endif
...
dscudaResult *
//dscudafreeid_1_svc(RCadr mem, struct svc_req *) // moikawa
dscudafreeid_1_svc(RCadr mem, struct svc_req *sr)
{
    static dscudaResult res;
    cudaError_t err;
...
#if 0
    //<--- moikawa
    fprintf(stderr, "%s, %s, cudaFree() returned %d\n", __FILE__, __func__, err);
    //---> moikawa
#endif
    return &res;
}
