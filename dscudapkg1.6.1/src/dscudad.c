...
response_to_search(void *arg) {
  ...
  while (1) {
    ...
    recvfrom(sock, buf, 256 - 1, 0, (struct sockaddr *)&clt, &sin_size);
    if (strcmp(buf, "DSCUDA_SEARCHSERVER") != 0)
      continue;

    WARN(2, "received DSCUDA_SEARCHSERVER message from %s\n", inet_ntoa(clt.sin_addr));
    clt.sin_family = AF_INET;
    clt.sin_port = htons(RC_DAEMON_IP_PORT - 2);
    inet_aton(inet_ntoa(clt.sin_addr), &(clt.sin_addr));
    sendto(sock, "DSCUDA_SERVERRESPONSE", 21, 0, (struct sockaddr *)&clt, sizeof(struct sockaddr));
    memset(buf, 0, 256);
    }
