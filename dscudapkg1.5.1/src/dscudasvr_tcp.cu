...
handleARemoteCall(int sock)
{
    ...
    if (!recvbuf) {
        recvbuf = (unsigned char *)calloc(RC_RDMA_BUF_SIZE, 1);
        sendbuf = (unsigned char *)calloc(RC_RDMA_BUF_SIZE, 1);
        if (!recvbuf || !sendbuf) {
    ...
    recv(sock, msg, sizeof(RCHdr), MSG_PEEK);
    pktsize = ((RCHdr *)msg)->payload;

    // now actually recv the packet.
    recv(sock, recvbuf, pktsize, 0);
    rpkt = (RCHdr *)recvbuf;
    spkt = (RCHdr *)sendbuf;
    ...
    nsent = send(sock, sendbuf, spkt->payload, 0);
    if (nsent < 0) {
        sprintf(msg, "handleAremoteCall():send() spkt->size:%d  nsent:%d",
                spkt->payload, nsent);
        perror(msg);
    }
}
