...
static int ibvUnpackKernelParam(CUfunction *kfuncp, int narg, RCArg *args);
static void setupIbv(void);

static void accept_p2p_connection(P2PConnection *p2pconn);
static int ibvDscudaSendP2P(RCHdr *rpkt0, RCHdr *spkt0);
static int ibvDscudaRecvP2P(RCHdr *rpkt0, RCHdr *spkt0);
...
static int
ibvUnpackKernelParam(CUfunction *kfuncp, int narg, RCArg *args)
{
    CUresult cuerr;
