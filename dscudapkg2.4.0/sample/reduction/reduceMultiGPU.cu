...
prepareDevices(int ndev_req, int nelem, int nblock)
{
    ...
    for (idev = 0; idev < ndev; idev++) {
        ...
        if (dev->nelem0 < dev->nelem) {
            if (dev->nelem0) {
                safeCall(cudaFree(dev->idata));
            }
            dev->nelem0 = dev->nelem;
            safeCall(cudaMalloc((void**)&dev->idata, sizeof(int) * dev->nelem));
        }
        if (dev->nblock0 < dev->nblock) {
            if (dev->nblock0) {
                safeCall(cudaFree((void *)dev->nblock));
            }
            dev->nblock0 = dev->nblock;
            safeCall(cudaMalloc((void**)&dev->odata, sizeof(int) * dev->nblock));
        }
...
reduceInEachDevice(int ndev, int *h_idata)
{
    ...
    for (idev = 0; idev < ndev; idev++) {
        ...
        safeCall(cudaMemcpy(dev->idata, h_idata + off, size, cudaMemcpyHostToDevice)); // !!!
#endif
        off += dev->nelem;
    }
...
retrieveResultsFromAllDevices(int ndev, int *h_odata)
{
    ...
    for (idev = 0; idev < ndev; idev++) {
        ...
        safeCall(cudaMemcpy(h_odata + off, dev->odata, size, cudaMemcpyDeviceToHost));
        off += dev->nblock;
    }
}
...
retrieveResultFromDevice(Device_t *dev, int *h_odata)
{
    ...
    size = sizeof(int) * dev->nblock;
    safeCall(cudaMemcpy(h_odata, dev->odata, size, cudaMemcpyDeviceToHost));
}
...
reduceDevicePair(Device_t *dev0, Device_t *dev1)
{
    ...
    if (!dev0->tmpdata) {
        safeCall(cudaMalloc((void**)&dev0->tmpdata, size));
    }
    safeCall(cudaMemcpy(dev0->tmpdata, dev1->odata, size, cudaMemcpyDefault));
