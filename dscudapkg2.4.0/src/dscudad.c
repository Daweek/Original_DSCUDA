...
spawn_server(int listening_sock)
{
    ...
    char *argv[16];
    char msg[256];
    char portstr[128], devstr[128], sockstr[128], svrname[512], exestr[512];
    int svrsize;
    char *svrimg;
    FILE *fp;
    ...
    recvMsgBySocket(sock, msg, sizeof(msg));
    sscanf(msg, "svrsize:%d", &svrsize); // dscudasvr size to be received.

    recvDataBySocket(sock, &svrimg, svrsize);
    sprintf(svrname, "%s/dscudasvr_p%d_d%d_S%d", Dscudasvrpath, sport, dev, sock);
    //    sprintf(svrname, "%s/dscudasvr", Dscudasvrpath);
            
    fp = fopen(svrname, "w");
    if (!fp) {
        perror("spawn_server:fopen.");
    }
    fwrite(svrimg, 1, svrsize, fp);
    fclose(fp);
    chmod(svrname, S_IRUSR | S_IWUSR | S_IXUSR);
