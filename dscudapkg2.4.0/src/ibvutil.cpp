...
#define WARN(lv, fmt, args...) if (lv <= dscudaWarnLevel()) {          \
        if (dscudaMyServerId() < 0) {                                   \
            fprintf(stderr, "client     : " fmt, dscudaMyServerId(), ## args); \
        }                                                               \
        else {                                                          \
...
typedef unsigned int RCMethod;

//        fprintf(stderr, "[%d]" fmt, dscudaMyServerId(), ## args);
...
check_msgsanity(struct ibv_wc *wc, bool is_server)
{
    ...
    switch (wc->opcode) {
      ...
      case IBV_WC_RECV_RDMA_WITH_IMM:
        WARN(0, "completion RECV RDMA w/IMM\n");
        break;
      default:
...
static void
on_completion_server(struct ibv_wc *wc)
{
    on_completion(wc, true);
}

static void
on_completion_client(struct ibv_wc *wc)
{
    ...
    switch (wc->opcode) {
      case IBV_WC_RECV:
        for (i = 0; i < RC_NWR_PER_POST; i++) {
            ...
            WARN(5, "peer_mr[%d]  adr:%lx  rkey:%x  lkey:%x\n",
        ...
        pthread_mutex_lock(&conn->rdma_mutex);
        conn->rdma_state = STATE_READY;
        pthread_mutex_unlock(&conn->rdma_mutex);
        //        post_recv_msg(conn); // rearm for next message. !!! this is useless.
        break;
      ...
      case IBV_WC_RECV_RDMA_WITH_IMM:
        WARN(3, "completion (rdma w/imm).\n");
        conn->rdma_imm_recvd = 1;
        rdmaPostReceive(conn);
        break;
...
struct ibv_mr *
rdmaRegisterMR(struct ibv_pd *pd, void *adr, int length, int is_send)
{
    struct ibv_mr *mr;
    ...
    TEST_Z(mr = ibv_reg_mr(pd, adr, length, access));

    WARN(3, "rdmRegisterMR  mr:0x%08llx  lkey:0x%08llx  rkey:0x%08llx  length:%d  addr:0x%08llx   pd:0x%08llx\n",
         mr, mr->lkey, mr->rkey, length, adr, pd);
...
register_memory(IbvConnection *conn)
{
    ...
    for (i = 0; i < RC_NWR_PER_POST; i++) {
        ...
        TEST_Z(conn->rdma_remote_mr[i] = ibv_reg_mr(conn->pd, 
                                                    ...
                                                    RC_SGE_SIZE, 
                                                    IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE));
        WARN(5, "set  remote_mr[%d]  adr:%lx  rkey:%x  lkey:%x\n",
...
post_send_msg(IbvConnection *conn)
{
         ...
         sge.addr, sge.length, sge.lkey);
    {
        ...
        for (i = 0; i < RC_NWR_PER_POST; i++) {
            WARN(5, "send remote_mr[%d]  adr:%lx  rkey:%x  lkey:%x\n",
                 i, mp->mr[i].addr, mp->mr[i].rkey, mp->mr[i].lkey);
        }
    }

    TEST_NZ(ibv_post_send(conn->qp, &wr, &bad_wr));
    // WARN(0, "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  posted a send WR.\n"); // !!!
}
...
kickoff_rdma_with_offset(intptr_t offset, IbvConnection *conn, int length)
{
    ...
    WARN(5, "i_start:%d  i_end:%d  offset:%d  length:%d\n",
    ...
    for (i = i_start; i < i_end; i++) {
        ...
        WARN(5, "radr:0x%lx  rkey:0x%x  ladr:0x%lx  lkey:0x%x  length:%d\n",
...
rdmaBuildConnection(struct rdma_cm_id *id, bool is_server)
{
    ...
    if (is_server) {
        conn->on_completion_handler = on_completion_server;
    }
    else {
        conn->on_completion_handler = on_completion_client;
    }
    pthread_mutex_init(&conn->rdma_mutex, NULL);
...
#if 0
    {
        ...
        at->timeout = 0;
        ibv_modify_qp(conn->qp, at, IBV_QP_TIMEOUT);
    }
#endif
...
rdmaBuildParams(struct rdma_conn_param *params)
{
    ...
    params->rnr_retry_count = 255; // uint8_t !!!
}
...
void
rdmaWaitRDMAImmRecvd(IbvConnection *conn)
{
    while (!conn->rdma_imm_recvd) {
        // nop.
    }
    conn->rdma_imm_recvd = 0; // clear for the next message.
}
...
void
rdmaWaitEvent(struct rdma_event_channel *ec, rdma_cm_event_type et,
              int (*handler)(struct rdma_cm_id *id))
{
...
rdmaPipelinedKickoff(IbvConnection *conn, int length,
                     char *payload_buf, char *payload_src, int payload_size, int chunk_size)
{
    int i;
    int size = chunk_size;
    int offset = sizeof(RCMethod); 
    ...
    int psize = (offset + chunk_size) - hdrsize;
    static int cnt = 0;

    pthread_mutex_lock(&conn->rdma_mutex);
    conn->rdma_nreq_pending = 0;
    conn->rdma_state = STATE_BUSY;
    for (i = 0; offset < length; i++) {
...
void
rdmaImmKickoff(IbvConnection *conn,
               int my_lkey, void *src,
               int peer_rkey, void *dst,
               int length)
{
    struct ibv_send_wr wr, *bad_wr = NULL;
    struct ibv_sge sge;
    ...
    wr.num_sge = 1;
    wr.send_flags = IBV_SEND_SIGNALED;
    wr.imm_data = 123; // an arbitrary immediate value (not used).
    ...
    WARN(3, "rdmaImmKickoff  lkey:0x%08llx  rkey:0x%08llx  length:%d  addr:0x%08llx  pd:0x%08llx\n",
         my_lkey, peer_rkey, length, src, conn->pd);
...
// For RDMA w/IMM only.
void
rdmaPostReceive(IbvConnection *conn)
{
    struct ibv_recv_wr wr, *bad_wr = NULL;
    memset(&wr, 0, sizeof(wr));
    wr.wr_id = (uintptr_t)conn;
