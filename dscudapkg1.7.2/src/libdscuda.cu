...
RCuvaRegister(int devid, void *adr[], size_t size)
{
    ...
    //    static int cnt = 0;
    //    fprintf(stderr, ">>> uva:0x%08lx  cnt:%d\n", uva, cnt++);
...
RCuvaUnregister(void *adr)
{
    ...
    //    static int cnt = 0;
    //    fprintf(stderr, "free(0x%08lx)   cnt:%d\n", uva, cnt++);
    //    showuva();

    // !!! this is commented out only for debugging purpose.
    // !!! must be activated in a productive run.
    free(uva);
}
