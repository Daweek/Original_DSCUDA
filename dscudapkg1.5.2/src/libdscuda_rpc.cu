...
setupConnection(int idev, RCServer_t *sp)
{
    ...
    if (!Clnt[idev][id]) {
        ...
        if (sp->ip == dscudaServerNameToAddr((char *)DEFAULT_SVRIP)) {
...
{
    if (rp) return;
    clnt_perror(Clnt[Vdevid[vdevidIndex()]][sp->id], dscudaAddrToServerIpStr(sp->ip));
    exit(1);
}
...
cudaConfigureCall(dim3 gridDim, dim3 blockDim, size_t sharedMem, cudaStream_t stream)
{
    ...
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudaconfigurecallid_1(gdim, bdim, sharedMem, (RCstream)stream, clnt[sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
#endif
{
    ...
    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudalaunchid_1((RCadr)func, clnt[sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
cudaSetupArgument(const void *arg, size_t size, size_t offset)
{
    ...
    argbuf.RCbuf_len = size;
    argbuf.RCbuf_val = (char *)arg;
    sp = vdev->server;

    for (int i = 0; i < vdev->nredundancy; i++, sp++) {
        rp = dscudasetupargumentid_1(argbuf, size, offset, clnt[sp->id]);
        checkResult(rp, sp);
        if (rp->err != cudaSuccess) {
            err = (cudaError_t)rp->err;
        }
...
extern "C" {
    ...
    __cudaRegisterFatBinary(void *fatCubin)
    {
        ...
        dscudaRegisterFatBinaryResult *rp;
        int vdevid;
        Vdev_t *vdev;
        CLIENT **clnt;
        RCServer_t *sp;
        RCbuf imgbuf;
        // int count = imgword * 8 + 0;
        ...
        imgbuf.RCbuf_len = count;
        imgbuf.RCbuf_val = (char *)img;

        for (int i = 0; i < vdev->nredundancy; i++, sp++) {
            rp = dscudaregisterfatbinaryid_1(fdtp->m, fdtp->v, imgbuf,
                                             fdtp->f ? fdtp->f : (char *)"", clnt[sp->id]);
            checkResult(rp, sp);
            if (i == 0) {
                handle = (void **)rp->handle;
            }
            xdr_free((xdrproc_t)xdr_dscudaRegisterFatBinaryResult, (char *)rp);
        }
        WARN(3, "done. handle:0x%llx\n\n", handle);
    ...
    __cudaUnregisterFatBinary(void **fatCubinHandle)
    {
        ...
        WARN(2, "__cudaUnregisterFatBinary(0x%016llx).\n", fatCubinHandle);
        ...
        for (int i = 0; i < vdev->nredundancy; i++, sp++) {
            rp = dscudaunregisterfatbinaryid_1((RCadr)fatCubinHandle, clnt[sp->id]);
            checkResult(rp, sp);
            xdr_free((xdrproc_t)xdr_dscudaResult, (char *)rp);
        }
    }
                           ...
                           int        *wSize)
    {
        ...
        WARN(2, "__cudaRegisterFunction(0x%llx, 0x%llx, %s, %s, %d, ...).\n",
        ...
        for (int i = 0; i < vdev->nredundancy; i++, sp++) {
            rp = dscudaregisterfunctionid_1((RCadr)fatCubinHandle,
                                            (RCadr)hostFun, deviceFun, (char *)deviceName,
                                            thread_limit, clnt[sp->id]);                                            
            checkResult(rp, sp);
            xdr_free((xdrproc_t)xdr_dscudaResult, (char *)rp);
        }
    }
