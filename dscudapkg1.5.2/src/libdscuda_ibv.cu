...
extern "C" {
    ...
    __cudaRegisterFatBinary(void *fatCubin)
    {
        ...
        int i, srcbyte, srcword;

        fprintf(stderr, "__cudaRegisterFatBinary(0x%016llx).\n", fatCubin);

        unsigned long long int *srcbuf = ((fatDeviceText_t *)fatCubin)->d;
        srcbyte = srcbuf[1];
        srcword = srcbyte / 8 + 16;
        //        for (i = 0; i < srcword; i += 4) {
        for (i = 0; i < 3 * 4; i += 4) {
            fprintf(stderr, "%d: %016llx %016llx %016llx %016llx\n",
                    i, srcbuf[i + 0], srcbuf[i + 1], srcbuf[i + 2], srcbuf[i + 3]);
        }
#endif
        return NULL;
    }
