...
typedef struct {
    ...
    char deviceFuncSymbol[128];
} KfuncEntry;
static int Nkfunc = 0; // # of kernel functions registered.
static KfuncEntry Kfunc[128];
...
dscudaRegisterFatBinaryResult *
dscudaregisterfatbinaryid_1_svc(int m, int v, RCbuf imgbuf, char *symbol, struct svc_req *sr)
{
    static dscudaRegisterFatBinaryResult res;
    void **handle = NULL;
    ...
    WARN(3, "dscudaRegisterFatBinary(%d, %d, 0x%llx, 0x%llx) done. size:%d symbol:%s\n",
         m, v, imgbuf.RCbuf_val, symbol, imgbuf.RCbuf_len, symbol);

    if (!dscuContext) createDscuContext();

#if 0
    { // use __cudaRegisterFatBinary().

        fatDeviceText_t *fdt = (fatDeviceText_t *)calloc(sizeof(fatDeviceText_t), 1);

        fdt->m = m;
        fdt->v = v;
        fdt->d = (unsigned long long *)calloc(imgbuf.RCbuf_len, 1);
        memcpy(fdt->d, imgbuf.RCbuf_val, imgbuf.RCbuf_len);
        fdt->f = symbol;
        handle = __cudaRegisterFatBinary(fdt);

        WARN(3, "size: 0x%llx byte, 0x%llx word\n", imgbuf.RCbuf_len, imgbuf.RCbuf_len / 8);

        int i;
        unsigned long long int *img = fdt->d;
        for (i = 0; i < 3 * 4; i += 4) {
            fprintf(stderr, "%d: %016llx %016llx %016llx %016llx\n",
                    i, img[i + 0], img[i + 1], img[i + 2], img[i + 3]);
        }
    }
#endif

#if 1 // use cuModuleLoadFatBinary().

        CUresult cuerr;

        // this makes cuModuleLoadFatBinary() work for unknown reason.
        cudaError_t err;
        int *dum;
        err = cudaMalloc((void**)&dum, 128);
        check_cuda_error(err);

        cuerr = cuModuleLoadFatBinary((CUmodule *)&handle, imgbuf.RCbuf_val);
        if (cuerr != CUDA_SUCCESS) {
...
dscudaResult *
dscudaunregisterfatbinaryid_1_svc(RCadr fatCubinHandle, struct svc_req *sr)
{
...
dscudaResult *
dscudaregisterfunctionid_1_svc(RCadr fatCubinHandle, RCadr hostFun, char *deviceFun, char *deviceName,
                               int thread_limit, struct svc_req *sr)
{
    static dscudaResult res;
    ...
    CUresult cuerr;
    cuerr = cuModuleGetFunction(&kfunchandle, (CUmodule)fatCubinHandle, deviceFun);
    if (cuerr == CUDA_SUCCESS) {
        WARN(3, "cuModuleGetFunction() : function '%s' found.\n", deviceFun);
    }
    else {
        WARN(0, "cuModuleGetFunction() : function:'%s'. %s\n",
             deviceFun, cudaGetErrorString((cudaError_t)cuerr));
        fatal_error(1);
    }

    Kfunc[Nkfunc].hostFuncPtr = (void *)hostFun;
    Kfunc[Nkfunc].hostFuncHandle = kfunchandle;
    memcpy(Kfunc[Nkfunc].deviceFuncSymbol, deviceFun, strlen(deviceFun) + 1);
    Nkfunc++;

#else
    __cudaRegisterFunction((void **)fatCubinHandle,
                           (const char *)hostFun, deviceFun, deviceName,
                           thread_limit, 0, 0, 0, 0, 0);
#endif
...
#if 1 // !!! use driver APIs instead of native cudaLaunch().

dscudaResult *
dscudalaunchid_1_svc(RCadr func, struct svc_req *sr)
{
    CUresult cuerr;
    cudaError_t err;
    int i;
    ...
    bool found = false;
    for (i = 0; i < Nkfunc; i++) {
        if ((void *)func == Kfunc[i].hostFuncPtr) {
            found = true;
            break;
        }
    }
    ...
    WARN(0, "Kfunc[%d]\n", i);
    WARN(0, "  hostFuncPtr      : 0x%016llx\n", func);
    WARN(0, "  hostFuncHandle   : 0x%016llx\n", kfunc);
    WARN(0, "  deviceFuncSymbol : %s\n", kname);
...
dscudaResult *
dscudaconfigurecallid_1_svc(RCdim3 gridDim, RCdim3 blockDim, RCsize sharedMem, RCstream stream, struct svc_req *sr)
{
    cudaError_t err;
    static dscudaResult res;
    dim3 gdim, bdim;
...
dscudaResult *
dscudasetupargumentid_1_svc(RCbuf argbuf, RCsize size, RCsize offset, struct svc_req *sr)
{
    CUresult cuerr;
    static dscudaResult res;

    WARN(3, "cudaSetupArgument(");
    WARN(3, "0x%llx %d, %d) done.\n", argbuf.RCbuf_val, size, offset);
    ...
    memcpy(Kparam[Nkparam].val, argbuf.RCbuf_val, size);
...
dscudaconfigurecallid_1_svc(RCdim3 gridDim, RCdim3 blockDim, RCsize sharedMem, RCstream stream, struct svc_req *sr)
{
    ...
    err = cudaConfigureCall(gdim, bdim, sharedMem, (cudaStream_t)stream);
    check_cuda_error(err);
    res.err = err;
    WARN(3, "[%d, %d, %d], [%d, %d, %d], %d, %d) done.\n",
         gridDim.x, gridDim.y, gridDim.z,
...
dscudasetupargumentid_1_svc(RCbuf argbuf, RCsize size, RCsize offset, struct svc_req *sr)
{
    ...
    err = cudaSetupArgument(argbuf.RCbuf_val, size, offset);
    check_cuda_error(err);
    res.err = err;
    WARN(3, "0x%llx, %d, %d) done.\n", argbuf.RCbuf_val, size, offset);
    return &res;
...
#endif // use driver APIs or cudaLaunch().
