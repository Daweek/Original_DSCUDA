...
struct dscudaRegisterFatBinaryResult {
    RCerror err;
    RCadr handle;
};
...
program DSCUDA_PROG {
    version DSCUDA_VER {
        ...
        /* Undocumenterd APIs */
        dscudaRegisterFatBinaryResult    dscudaRegisterFatBinaryId(int m, int v, RCbuf imgbuf, string symbol) = 1;
        dscudaResult                     dscudaUnregisterFatBinaryId(RCadr fatCubinHandle) = 2;
        dscudaResult                     dscudaRegisterFunctionId(RCadr fatCubinHandle, RCadr hostFun,
                                                                  string deviceFun, string deviceName,
                                                                  int thread_limit) = 3;
        /* Thread Management */
	...
	dscudaResult                     dscudaDeviceReset(void) = 309;
        ...
        dscudaResult                     dscudaConfigureCallId(RCdim3 gridDim, RCdim3 blockDim, RCsize sharedMem, RCstream stream) = 603;
        dscudaResult                     dscudaLaunchId(RCadr func) = 604;
        dscudaResult                     dscudaSetupArgumentId(RCbuf argbuf, RCsize size, RCsize offset) = 605;
