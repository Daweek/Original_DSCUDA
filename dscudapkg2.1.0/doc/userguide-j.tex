...
\vspace*{5mm}
{\LARGE for DS-CUDA version 2.1.0}\\

\vspace*{10mm}
{\large 最終更新 : 2014 年 8 月 11 日
}
\end{center}
...
\begin{tabular}{ll}
  ...
  DSCUDA\_SVRPATH      : & DS-CUDA サーバプログラムの実行ファイルがおかれているパス\\
                         & (client node)、おかれるパス (server node)。\\[3mm]
  \hline
  server node のみ       & \\
  \hline
...
\item CUDA ランタイムライブラリのダミー: {\tt \$dscudapkg/bin/libcudart.so.3}

\item DS-CUDA サーバライブラリ: {\tt \$dscudapkg/bin/libdscudasvr.a}
...
従来の DS-CUDA では、以下の手順でアプリケーションプログラムを生成、実行していました。

\begin{enumerate}
\renewcommand{\labelenumi}{\arabic{enumi}) }
\item アプリケーションプログラムのソースコードを {\tt dscudacpp} を用い
    てコンパイルし、実行ファイルとデバイスコード (PTX データ) を生成する。
\item server node 上で DS-CUDA デーモンを起動する。
\item client node 上で アプリケーションプログラムを実行する。
\item アプリケーションプログラムからの要求に応じて DS-CUDA デーモンが DS-CUDA サーバを起動する。
\item アプリケーションプログラムと DS-CUDA サーバが通信を確立し、処理をすすめる。
\end{enumerate}

CUDA カーネル (デバイスコード) は、アプリケーションプログラムの実行時
に client node から server node へ転送されていました。より具体的に
は、client node 上の {\tt PTX} データ) がアプリケーションプログラムから
サーバプログラムへ転送され、サーバプログラムはこのデータを動的にロード、
実行していました。
...
をサポートしました。バージョン 2.0.0 では以下の手順でアプリケーションプ
ログラムが生成、実行されます。

\begin{enumerate}
\renewcommand{\labelenumi}{\arabic{enumi}) }
\item アプリケーションプログラムのソースコードを {\tt dscudacpp} を用い
    てコンパイルし、実行ファイルを生成する。同時にDS-CUDA サーバの実行ファイル {\tt dscudasvr} も生成する。
\item server node 上で DS-CUDA デーモンを起動する。
\item client node 上で アプリケーションプログラムを実行する。
\item アプリケーションプログラムは {\tt dscudasvr} を DS-CUDA デーモンへ送信する。
\item アプリケーションプログラムからの要求に応じて DS-CUDA デーモンは {\tt dscudasvr} を受信し、
それをファイルとして保存し、起動する。
\item アプリケーションプログラムと DS-CUDA サーバが通信を確立し、処理をすすめる。
\end{enumerate}

この方法ではアプリケーションごとに専用の DS-CUDA サーバが生成されます。
生成されたサーバは、アプリケーションプログラム実行時に server node へ送信されます。

従来にくらべて余分な準備手続きを必要としますが、Dynamic Parallelism を
使用するコードも扱えるという利点があります。CUDA カーネル (デバイスコー
ド) は DS-CUDA サーバに含まれており、アプリケーションプログラム実行時に
動的にロードする必要がありません。そのため CUDA ドライ
バ API が Dynamic Parallelism をサポートしていないことは問題になりません。

\Subsection{アプリケーションプログラムの生成}\label{sec:build}
...
は、client node 上で動作する通常の実行ファイル 2 種類 (TCP インタフェー
ス用と IBV インタフェース用) と、sercer node 上でサーバとして動作する実
行ファイル ({\tt dscudasvr}) が同時に生成されます。
...
kawai@client>ls
bar.o  bar.svr.o  dscudasvr  foo  foo.cu  foo.o  foo.svr.o  foo_ibv  foo_tcp
\end{verbatim}
\end{minipage}\\[5mm]
%
...
{\tt dscudacpp} がサーバ {\tt dscudasvr} を生成する際に使用する main()
...
デーモンはクライアントからの要求に応じて動的にサーバプログラムを起動します。
...
これから実行しようとするアプリケーションプログラム向けのサーバプログラム
{\tt dscudasvr} は、アプリケーションプログラムの起動時に client node から受信し、
server node 上の任意のディレクトリに配置できます。
...
\item Unified Virtual Addressing を前提としたメモリ操作 API。\\例: {\tt cudaMemcpy(..., cudaMemcpyDefault)}
\item GPUDirect 関連機能
\end{itemize}
%
...
アプリケーションプログラムが特定の CUDA カーネルの実行を DS-CUDA サーバ
へ要求する際、カーネルの同定はカーネル関数の名前を用いて行います。より
具体的には、{\tt g++} の組み込み関数 {\tt \_\_PRETTY\_FUNCTION\_\_} を
使用してカーネル関数の名前 (引数や C++ テンプレートパラメタの情報を含
む) を取得し、この文字列を DS-CUDA サーバへ通知します。

DS-CUDA サーバは受け取った {\tt \_\_PRETTY\_FUNCTION\_\_} の値から関数
...
\begin{verbatim}
  例)
    ...
    関数 signature         _ZN6foobarL7vecAddTIdEv
     ...
     ↓ nm dscudasvr | grep _ZN6foobarL7vecAddTIdEv
