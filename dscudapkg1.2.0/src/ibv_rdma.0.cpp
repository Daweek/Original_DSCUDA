    ...
    switch (wc->opcode) {
      ...
      case IBV_WC_RDMA_WRITE:
        switch (conn->rdma_state) {
          case STATE_READY:
            break;
          case STATE_BUSY0: // kickoff_rdma_with_offset(offset, ...) done.
            conn->rdma_nreq_pending--;
            //            conn->rdma_state = STATE_BUSY1;
            break;
          case STATE_BUSY1: // kickoff_rdma_with_offset(0, ...) done.
            conn->rdma_state = STATE_READY;
            break;
          default:
...
rdmaKickoff(IbvConnection *conn, int length)
{
    ...
    kickoff_rdma_with_offset(offset, conn, length - offset); // STATE_BUSY0
    conn->rdma_state = STATE_BUSY1;
    kickoff_rdma_with_offset(0, conn, offset); // STATE_BUSY1
}
                     ...
                     char *payload_buf, char *payload_src, int payload_size)
{
    ...
    // const int size_d = 200; // pipelined size
    //    const int size_d = 1000000000; // pipelined size
    uintptr_t at;
    int offset = sizeof(RCMethod); 
    int size = size_d;
    ...
    int psize = payload_size;

    if (offset + size_d < length) {
        psize = (offset + size_d) - hdrsize;
    }

    for (int i = 0; length > offset; i++) {
        ...
        kickoff_rdma_with_offset(offset, conn, size); // STATE_BUSY0
        offset += size_d;
        conn->rdma_nreq_pending++;
    }

    //    usleep(1000);
    while (conn->rdma_nreq_pending) {
        // nop.
    }
    conn->rdma_state = STATE_BUSY1;
    kickoff_rdma_with_offset(0, conn, sizeof(RCMethod)); // STATE_BUSY1
}
